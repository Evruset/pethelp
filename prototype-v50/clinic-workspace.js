...
(() => {
  ...
  function adminTabs() {
    const tabs = state.role === 'doctor'
      ? [
        ['clinic-workspace','dashboard','Моя смена'], ['clinic-patients','patients','Пациенты'],
        ['clinic-visit','visit','Приём'], ['clinic-telemed','telemed','Телемедицина']
      ]
      : [
        ...
        ['clinic-telemed','telemed','Онлайн-очередь']
      ];
  ...
  function dashboardPage() {
    if (state.role === 'doctor') return doctorDashboardPage();
  ...
  function doctorDashboardPage() {
    const mine = state.appointments.filter((item) => item.doctorId === 'anna' && !['done','cancelled'].includes(item.status));
    const ready = mine.filter((item) => ['checkedin','in-visit'].includes(item.status));
    const online = state.telemed.filter((item) => item.doctorId === 'anna' && ['waiting','scheduled','in-call'].includes(item.status));
    return shell('Моя смена','Пациенты и консультации Анны Смирновой — без административных операций клиники.',
      `<section class="v48-grid v48-grid--3" aria-label="Показатели врача">
        <article class="v48-panel v48-kpi"><div class="v48-kpi__top"><span class="v49-kpi-label">${icon('appointments')}<span class="v48-kpi__label">Очные приёмы</span></span><span class="v48-trend">Сегодня</span></div><strong class="v48-kpi__value">${mine.filter((item) => item.channel === 'clinic').length}</strong><span class="v48-kpi__hint">Только расписание текущего врача</span></article>
        <article class="v48-panel v48-kpi"><div class="v48-kpi__top"><span class="v49-kpi-label">${icon('patients')}<span class="v48-kpi__label">Готовы к осмотру</span></span><span class="v48-trend">Сейчас</span></div><strong class="v48-kpi__value">${ready.length}</strong><span class="v48-kpi__hint">Пациенты уже отмечены администратором</span></article>
        <article class="v48-panel v48-kpi"><div class="v48-kpi__top"><span class="v49-kpi-label">${icon('telemed')}<span class="v48-kpi__label">Онлайн-консультации</span></span><span class="v48-trend">Сегодня</span></div><strong class="v48-kpi__value">${online.length}</strong><span class="v48-kpi__hint">Ожидание, план и активная комната</span></article>
      </section>
      <div class="v48-grid v48-grid--dashboard" style="margin-top:14px">
        <section class="v48-panel"><div class="v48-panel-head"><div><h2>Следующие пациенты</h2><p>Приём открывается после отметки о прибытии на ресепшене.</p></div></div><div class="v48-task-list">${mine.slice(0,5).map((item) => { const patient=patientById(item.patientId); const canStart=['checkedin','in-visit'].includes(item.status); return `<article class="v48-task ${canStart ? '' : 'is-muted'}"><span class="v48-task__time">${item.time}</span><span><strong>${esc(patient.name)} · ${esc(item.service)}</strong><small>${esc(item.room)} · ${statusBadge(item.status)}</small></span>${canStart ? `<button class="v48-mini-button primary" type="button" data-appointment-action="visit" data-id="${item.id}">Открыть приём</button>` : `<span class="v48-status gray">Ожидает ресепшен</span>`}</article>`; }).join('')}</div></section>
        <aside class="v48-panel"><div class="v48-panel-head"><div><h2>Врачебные задачи</h2><p>Только медицинские действия текущей смены.</p></div></div><div class="v48-activity-list"><div class="v48-activity"><time>Сейчас</time><span><strong>Проверить аллергию Барни</strong><small>Амоксициллин и лидокаин требуют уточнения</small></span></div><div class="v48-activity"><time>12:00</time><span><strong>Онлайн-консультация Тоши</strong><small>Снижение аппетита · 2 документа</small></span></div><div class="v48-activity"><time>До 18:00</time><span><strong>Подписать черновики</strong><small>1 клиническая запись не завершена</small></span></div></div></aside>
      </div>`
    );
  }
  ...
  function selectedSlotPanel() {
    ...
    const appointment = appointmentById(slot.appointmentId);
    const patient = appointment ? patientById(appointment.patientId) : null;
    return `<div class="v48-slot-editor v50-slot-editor"><div class="v48-fact-grid v50-slot-facts">
      <div class="v48-fact"><span>Время</span><strong>${slot.time} · ${slot.duration} минут</strong></div>
      <div class="v48-fact v50-slot-resource"><span>Врач и ресурс</span><strong>${esc(resource.name)}</strong><small>${esc(resource.meta)}</small></div>
    ...
    ${patient ? `<div class="v48-person v50-slot-person"><img src="${patient.avatar}" alt=""/><span><strong>${esc(patient.name)}</strong><small>${esc(appointment.service)}</small><small>${esc(patient.owner)} · ${esc(patient.phone)}</small></span></div>` : `<p class="muted small">${esc(slot.label || 'Служебное окно')}</p>`}
    <div class="v48-panel-actions v50-slot-actions">${appointment ? `<button class="btn" type="button" data-open-appointment="${appointment.id}">Открыть запись</button>` : ''}<button class="btn secondary" type="button" data-slot-toggle="${slot.id}">${slot.status === 'blocked' ? 'Открыть слот' : 'Заблокировать окно'}</button></div></div>`;
  }
  ...
  function patientSummary(patient) {
    return `<div class="v48-patient-hero"><img src="${patient.avatar}" alt="${esc(patient.name)}"/><div><span class="kicker">${patient.id}</span><h2>${esc(patient.name)}</h2><p class="muted small">${esc(patient.species)} · ${esc(patient.breed)} · ${esc(patient.age)}</p></div></div>
      ...
      <div class="v48-panel-actions" style="margin-top:13px"><button class="btn ${state.role === 'doctor' ? 'secondary' : ''}" type="button" data-open-patient="${patient.id}">Открыть карточку</button>${state.role === 'doctor' ? `<button class="btn" type="button" data-start-visit="${patient.id}">Новый приём</button>` : `<a class="btn secondary" href="tel:${patient.phone.replace(/[^+\d]/g,'')}">Связаться</a>`}</div>`;
  }

  function visitPatientContext(patient) {
    return `<div class="v50-visit-patient">
      <div class="v48-patient-hero"><img src="${patient.avatar}" alt="${esc(patient.name)}"/><div><span class="kicker">${patient.id}</span><h2>${esc(patient.name)}</h2><p class="muted small">${esc(patient.species)} · ${esc(patient.breed)} · ${esc(patient.age)}</p></div></div>
      ${patient.allergies.length ? `<div class="v48-alert">${icon('allergy')}<span><strong>Аллергии и ограничения</strong><small>${esc(patient.allergies.join(' · '))}</small></span></div>` : ''}
      <dl class="v50-patient-facts"><div><dt>Владелец</dt><dd>${esc(patient.owner)}</dd></div><div><dt>Телефон</dt><dd>${esc(patient.phone)}</dd></div><div><dt>Вес</dt><dd>${esc(patient.weight)}</dd></div><div><dt>Последний визит</dt><dd>${esc(patient.lastVisit)}</dd></div><div><dt>Страхование</dt><dd>${esc(patient.policy)}</dd></div><div><dt>Документы</dt><dd>${patient.documents}</dd></div></dl>
      <button class="btn secondary full" type="button" data-open-patient="${patient.id}">Открыть полную карточку</button>
    </div>`;
  }
  ...
  function visitPage() {
    ...
    return shell('Приём и лечение','Структурированный черновик осмотра, назначения, согласия и завершение визита.',
      `<form id="v48-visit-form" class="v48-visit-layout" data-patient-id="${patient.id}">
        <aside class="v48-grid v48-visit-sidebar"><section class="v48-panel">${visitPatientContext(patient)}</section><section class="v48-panel v50-visit-progress"><div class="v48-panel-head"><div><h3>Ход приёма</h3><p>Административные статусы доступны ресепшену.</p></div></div><ol><li class="is-done"><span>1</span><div><strong>Пациент прибыл</strong><small>Отмечено ресепшеном · 09:52</small></div></li><li class="is-current"><span>2</span><div><strong>Осмотр и назначения</strong><small>Черновик сохраняется автоматически</small></div></li><li><span>3</span><div><strong>Заключение</strong><small>Подпись врача завершит визит</small></div></li></ol></section></aside>
  ...
  function telemedDetail() {
    let item = telemedById(state.selectedTelemed) || state.telemed[0];
    if (state.role === 'doctor' && item.doctorId !== 'anna') item = state.telemed.find((entry) => entry.doctorId === 'anna') || item;
    ...
    return `<div class="v48-panel-head"><div><h2>${esc(patient.name)} · ${esc(item.topic)}</h2><p>${esc(patient.owner)} · ${esc(patient.phone)}</p></div>${statusBadge(item.status)}</div>
      ...
      <div class="v48-panel-actions" style="margin-top:12px">${state.role === 'doctor' ? (item.status !== 'in-call' ? `<button class="btn" type="button" data-telemed-action="start" data-id="${item.id}">Начать консультацию</button>` : `<button class="btn" type="button" data-telemed-action="complete" data-id="${item.id}">Завершить</button>`) : `<button class="btn" type="button" data-prototype-action="assign-doctor">Назначить врача</button>`}<button class="btn secondary" type="button" data-telemed-action="route" data-id="${item.id}">Очный маршрут</button><a class="btn secondary" href="tel:${patient.phone.replace(/[^+\d]/g,'')}">Позвонить</a></div>`;
  }
  function telemedPage() {
    const subtitle = state.role === 'doctor' ? 'Мои онлайн-приёмы, клинические заметки и передача в очный маршрут.' : 'Распределение очереди, связь с владельцем и контроль времени ожидания.';
    return shell(state.role === 'doctor' ? 'Мои онлайн-приёмы' : 'Онлайн-очередь',subtitle,
      `<div class="v48-telemed-layout"><aside class="v48-panel"><div class="v48-panel-head"><div><h2>${state.role === 'doctor' ? 'Консультации Анны' : 'Очередь клиники'}</h2><p>${state.telemed.filter((item) => ['waiting','needs-route'].includes(item.status)).length} обращения требуют действия.</p></div><span class="v48-status green">4 врача онлайн</span></div><div class="v48-telemed-list">${state.telemed.filter((item) => state.role !== 'doctor' || item.doctorId === 'anna').map((item) => { const patient = patientById(item.patientId); const risky=item.redFlags !== 'Не отмечены'; return `<button class="v48-telemed-item ${state.selectedTelemed === item.id ? 'is-selected' : ''}" type="button" data-select-telemed="${item.id}"><img src="${patient.avatar}" alt=""/><span class="v50-telemed-copy"><strong>${esc(patient.name)}</strong><span>${esc(item.topic)}</span><small>${esc(item.wait)}${risky ? ` · ${esc(item.redFlags)}` : ' · Опасные признаки не отмечены'}</small></span><span class="v50-telemed-status">${statusBadge(item.status)}</span></button>`; }).join('')}</div></aside><section class="v48-panel">${telemedDetail()}</section></div>`
    );
  }
  ...
  function updateRouteChrome() {
    const route = currentRoute();
    const roleRoutes = state.role === 'doctor'
      ? new Set(['clinic-workspace','clinic-patients','clinic-patient','clinic-visit','clinic-telemed'])
      : new Set(['clinic-workspace','clinic-schedule','clinic-appointments','clinic-patients','clinic-patient','clinic-telemed']);
    if (ADMIN_ROUTES.has(route) && !roleRoutes.has(route)) { location.hash = '#clinic-workspace'; return; }
  ...
  function bindEvents() {
    document.addEventListener('click', (event) => {
      ...
      if (role) { state.role = role.dataset.adminRole; const allowed = state.role === 'doctor' ? new Set(['clinic-workspace','clinic-patients','clinic-patient','clinic-visit','clinic-telemed']) : new Set(['clinic-workspace','clinic-schedule','clinic-appointments','clinic-patients','clinic-patient','clinic-telemed']); addAudit(`Роль интерфейса: ${state.role === 'doctor' ? 'Врач' : 'Ресепшен'}`); persist(); renderAll(); if (!allowed.has(currentRoute())) location.hash = '#clinic-workspace'; toast(`Режим «${state.role === 'doctor' ? 'Врач' : 'Ресепшен'}»: показаны только доступные задачи.`); return; }
