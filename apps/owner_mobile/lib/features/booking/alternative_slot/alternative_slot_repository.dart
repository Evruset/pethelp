...
class AlternativeSlotSnapshot {
  ...
  DateTime authoritativeNow(DateTime deviceNow) =>
      deviceNow.toUtc().add(_serverNowOffset);
}
...
class AlternativeSlotRepository {
  ...
  Future<AlternativeSlotResult<AlternativeSlotSnapshot>> readSnapshot(
    ...
    if (response.statusCode == 401) {
      return const AlternativeSlotFenced('UNAUTHENTICATED');
    }
    if (response.statusCode == 403) {
      return const AlternativeSlotFenced('FORBIDDEN');
    }
    if (response.statusCode == 404 || response.statusCode == 422) {
      return AlternativeSlotFenced(_code(payload));
    }
  ...
  String _code(Map<String, dynamic> payload) =>
