...
class OwnerAppointmentPresentation {
  ...
  factory OwnerAppointmentPresentation.fromJson(dynamic json,
    ...
    return bucket == 'HISTORY'
        ? const OwnerAppointmentPresentation(
            ...
            label: 'Запись сохранена в истории',
            description: 'Обновите приложение, чтобы увидеть подробный статус.',
            tone: 'neutral')
        : const OwnerAppointmentPresentation(
            code: 'STATUS_SYNCING',
...
class OwnerAppointment {
  ...
  factory OwnerAppointment.fromJson(Map<String, dynamic> json) {
    ...
    return OwnerAppointment(
      ...
      bucket: bucket,
      presentation: OwnerAppointmentPresentation.fromJson(json['presentation'],
          bucket: bucket),
...
class HttpOwnerAppointmentsRepository implements OwnerAppointmentsRepository {
  ...
  Future<ReleasedBookingHold> releaseHold(String holdId) async {
    ...
    final response = await _client.post(
      ...
      headers: await _headers(
          idempotencyKey: _uuid.v4(), correlationId: correlationId),
    );
    final data = _decode(response);
    if (response.statusCode != 200 || data is! Map<String, dynamic>) {
      throw OwnerAppointmentsApiException(
