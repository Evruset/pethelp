...
const appointmentId = '55555555-5555-4555-8555-555555555555'; const petId = '66666666-6666-4666-8666-666666666666'; const visitId = '77777777-7777-4777-8777-777777777777';
...
test('completes a confirmed visit', async ({ page, context, baseURL }) => {
  ...
  await expect(page.getByText('Результат ещё не оформлен', { exact: true })).toBeVisible(); await expect(form).toHaveCount(0);
...
test('validates summary boundaries without posting', async ({ page, context, baseURL }) => {
  ...
  const tooLong = 'x'.repeat(8001); await field.evaluate((element, value) => { const setter = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value')?.set; setter?.call(element, value); element.dispatchEvent(new Event('input', { bubbles: true })); }, tooLong); await button.click(); await expect(page.locator('#completion-summary-message')).toHaveText('Введите не более 8000 символов.'); await expect(field).toHaveValue(tooLong); await expect(field).toBeFocused();
  expect(postReads).toBe(0);
});

test('accepts 3 and 8000 character summaries', async ({ page, context, baseURL }) => {
  await open(page, context, baseURL); await page.getByLabel('Клиническое заключение').fill('xyz'); await page.getByRole('button', { name: 'Завершить приём' }).click(); await expect(page.getByText('Результат ещё не оформлен', { exact: true })).toBeVisible(); expect(postReads).toBe(1);
  status = 'CONFIRMED'; postReads = 0; detailReads = 0; await open(page, context, baseURL); await page.getByLabel('Клиническое заключение').fill('x'.repeat(8000)); await page.getByRole('button', { name: 'Завершить приём' }).click(); await expect(page.getByText('Результат ещё не оформлен', { exact: true })).toBeVisible(); expect(postReads).toBe(1);
});
...
test('pending double click and Enter submit exactly once', async ({ page, context, baseURL }) => {
  ...
  await expect(page.getByText('Результат ещё не оформлен', { exact: true })).toBeVisible();
  expect(postReads).toBe(1); expect(detailReads).toBe(2);
});
...
test('idempotent, validation, forbidden, conflict, failure and network responses are controlled', async ({ page, context, baseURL }) => {
  for (const responseMode of ['idempotent', 'validation', 'forbidden', 'conflict', 'failure', 'network'] as const) {
    ...
    if (responseMode === 'idempotent' || responseMode === 'conflict') { await expect(page.getByText('Результат ещё не оформлен', { exact: true })).toBeVisible(); expect(detailReads).toBe(2); }
    else { await expect(page.locator('#completion-summary-message')).toBeVisible(); await expect(field).toHaveValue('summary'); expect(detailReads).toBe(1); }
  }
});
...
test('completion states have labels, live errors and scoped axe coverage', async ({ page, context, baseURL }) => {
  ...
  mode = 'success'; await open(page, context, baseURL); await page.getByLabel('Клиническое заключение').fill('done'); await page.getByRole('button', { name: 'Завершить приём' }).click(); await expect(page.getByText('Результат ещё не оформлен', { exact: true })).toBeVisible(); await expect((await new AxeBuilder({ page }).include('[aria-labelledby="vet-visit-detail-title"]').analyze()).violations).toEqual([]);
});
...
function visit() { return { holdId, clinicId, locationId, scheduledStart: '2026-07-12T10:00:00.000Z', scheduledEnd: '2026-07-12T10:30:00.000Z', status, petDisplayName: 'Milo', species: 'CAT', appointmentId, petId, visitId: status === 'COMPLETED' ? visitId : null }; }
