...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const body = await request.json().catch(() => null) as { noteRequest?: unknown } | null;
  ...
  try {
    const upstream = await fetch(`${backendBaseUrl()}/v1/clinic/booking-holds/${holdId}/request-notes`, {
      method: 'POST',
      headers: {
