...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  if (typeof body?.periodType !== 'string' || typeof body.startsAt !== 'string' || typeof body.endsAt !== 'string') {
    return NextResponse.json({ code: 'INVALID_REQUEST' }, { status: 400 });
  }

  try {
    const upstream = await fetch(`${backendBaseUrl()}/v1/clinic/${clinicId}/locations/${locationId}/schedule/periods`, {
      method: 'POST',
      headers: {
