...
const bodyIsValid = (value: unknown): value is { alias: string | null } => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const item = value as Record<string, unknown>;
  return Object.keys(item).length === 1 && Object.hasOwn(item, 'alias') && (item.alias === null || typeof item.alias === 'string');
};
...
export async function PATCH(request: Request, context: Context): Promise<NextResponse> {
  ...
  if (!bodyIsValid(payload)) return safe('INVALID_PATIENT_ALIAS', 422);
  try {
    ...
    const response = await fetch(`${backend()}/v1/clinic/${encodeURIComponent(clinicId)}/locations/${encodeURIComponent(locationId)}/patients/${encodeURIComponent(patientId)}/local-profile`, {
    ...
    const allowed = new Set([
      ...
      'PATIENT_ASSOCIATION_CHANGED', 'IDEMPOTENCY_KEY_REUSED', 'INVALID_PATIENT_ALIAS',
      'PRECONDITION_REQUIRED', 'POLICY_TEMPORARILY_UNAVAILABLE',
    ]);
