...
function scopeDenied(): NextResponse {
...
export async function GET(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const upstream = new URL(`${backendBaseUrl()}/v1/clinic/${clinicId}/locations/${locationId}/schedule/slots`);
  upstream.searchParams.set('from', incoming.searchParams.get('from') ?? new Date().toISOString());
  upstream.searchParams.set('to', incoming.searchParams.get('to') ?? new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString());
