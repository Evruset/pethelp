...
import { startTelemedCaseSession, TelemedVetBackendError } from '@/lib/api/telemed-vet';
...
export async function POST(_: Request, context: RouteContext): Promise<NextResponse> {
  ...
  try {
    return NextResponse.json(await startTelemedCaseSession(session, clinicId, locationId, caseId));
  } catch (error) {
