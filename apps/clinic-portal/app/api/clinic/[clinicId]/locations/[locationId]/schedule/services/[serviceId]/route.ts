...
type RouteContext = {
  params: Promise<{ clinicId: string; locationId: string; serviceId: string }>;
};
...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  const { clinicId, locationId, serviceId } = await context.params;
  const session = await getClinicSession();
  if (!session || !UUID.test(clinicId) || !UUID.test(locationId) || !UUID.test(serviceId) || !canAccessClinicLocation(session, clinicId, locationId)) {
  ...
  try {
    const upstream = await fetch(`${backendBaseUrl()}/v1/clinic/${clinicId}/locations/${locationId}/schedule/services/${serviceId}`, {
      method: 'POST',
      headers: {
