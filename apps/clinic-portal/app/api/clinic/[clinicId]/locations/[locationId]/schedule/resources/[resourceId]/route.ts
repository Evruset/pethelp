...
type RouteContext = {
  params: Promise<{ clinicId: string; locationId: string; resourceId: string }>;
};
...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  const { clinicId, locationId, resourceId } = await context.params;
  const session = await getClinicSession();
  if (!session || !UUID.test(clinicId) || !UUID.test(locationId) || !UUID.test(resourceId) || !canAccessClinicLocation(session, clinicId, locationId)) {
  ...
  const body = await request.json().catch(() => null) as { code?: unknown; displayName?: unknown; resourceType?: unknown; active?: unknown } | null;
  if (typeof body?.code !== 'string' || typeof body.displayName !== 'string' || typeof body.resourceType !== 'string') {
    return NextResponse.json({ code: 'INVALID_REQUEST' }, { status: 400 });
  }

  try {
    const upstream = await fetch(`${backendBaseUrl()}/v1/clinic/${clinicId}/locations/${locationId}/schedule/resources/${resourceId}`, {
      method: 'POST',
      headers: {
