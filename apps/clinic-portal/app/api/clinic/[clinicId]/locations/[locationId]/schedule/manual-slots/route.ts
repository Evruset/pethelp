...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const body = await request.json().catch(() => null) as { serviceId?: unknown; startsAt?: unknown; endsAt?: unknown; capacity?: unknown } | null;
  if (typeof body?.serviceId !== 'string' || !UUID.test(body.serviceId) || typeof body.startsAt !== 'string' || typeof body.endsAt !== 'string') {
    return NextResponse.json({ code: 'INVALID_REQUEST' }, { status: 400 });
  }

  try {
    const upstream = await fetch(`${backendBaseUrl()}/v1/clinic/${clinicId}/locations/${locationId}/schedule/manual-slots`, {
      method: 'POST',
      headers: {
