...
type RouteContext = {
  params: Promise<{ clinicId: string; locationId: string; periodId: string }>;
};
...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  const { clinicId, locationId, periodId } = await context.params;
  const session = await getClinicSession();
  if (!session || !UUID.test(clinicId) || !UUID.test(locationId) || !UUID.test(periodId) || !canAccessClinicLocation(session, clinicId, locationId)) {
  ...
  try {
    const upstream = await fetch(`${backendBaseUrl()}/v1/clinic/${clinicId}/locations/${locationId}/schedule/periods/${periodId}/cancel`, {
      method: 'POST',
      headers: {
