...
export default defineConfig({
  testDir: './tests/e2e',
  outputDir: './test-results/owner-expo-web-real-stack',
  fullyParallel: false,
  workers: 1,
  retries: 0,
  reporter: [['list']],
  ...
  webServer: [
    {
      ...
      env: {
        ...
        OWNER_WEB_BFF_IP_SIGNING_SECRET: process.env.OWNER_WEB_BFF_IP_SIGNING_SECRET ?? 'local-owner-web-ip-signing-secret-32-bytes',
        PATH: process.env.PATH ?? '',
      },
    },
    {
      command: `cd ../clinic-portal && npm run build && npm run start -- -H 127.0.0.1 -p ${clinicPort}`,
      url: clinicOrigin,
      reuseExistingServer: !process.env.CI,
      timeout: 180_000,
      env: {
        VETHELP_API_BASE_URL: process.env.VETHELP_API_BASE_URL ?? 'http://127.0.0.1:3000',
        VETHELP_CLINIC_JWT_SECRET: process.env.VETHELP_CLINIC_JWT_SECRET ?? 'local-development-jwt-signing-key-not-for-shared-use',
