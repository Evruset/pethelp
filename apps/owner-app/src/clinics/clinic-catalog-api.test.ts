...
describe('clinic catalog api',()=>{
  it('sends authority and accepts the bounded response',async()=>{
    ...
    await expect(createClinicCatalogApi({request} as ApiClient).list('credential','  Clinic   Address  ')).resolves.toMatchObject({clinics:[{name:'Clinic'}]});
    expect(request).toHaveBeenCalledWith('v1/owner/clinic-catalog?q=Clinic%20Address',expect.objectContaining({headers:{Authorization:'Bearer credential'}}));
    await createClinicCatalogApi({request} as ApiClient).list('credential','   ');
