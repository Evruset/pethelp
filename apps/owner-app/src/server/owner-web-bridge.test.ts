...
describe('Expo Owner Web session bridge', () => {
  ...
  it('forwards only allowlisted specialist discovery GET query parameters with the HttpOnly session',async()=>{jest.spyOn(global,'fetch').mockResolvedValue(new Response(JSON.stringify({observedAt:'2026-08-29T08:00:00.000Z',limit:50,doctors:[]}),{status:200}));const cookie=Buffer.from(JSON.stringify({credential:TOKEN,expiresAt:EXPIRES_AT}),'utf8').toString('base64url');const response=await ownerWebBridge(request('v1/owner/clinic-catalog/specialist-discovery?serviceCode=PRIMARY_EXAM&limit=50',{headers:{cookie:`__Host-vethelp_owner_session=${cookie}`}}),['v1','owner','clinic-catalog','specialist-discovery']);expect(response.status).toBe(200);const [url,init]=(global.fetch as jest.Mock).mock.calls[0];expect(String(url)).toBe('http://backend.test/v1/owner/clinic-catalog/specialist-discovery?serviceCode=PRIMARY_EXAM&limit=50');expect(init.headers.authorization).toBe(`Bearer ${TOKEN}`);});
  it('allowlists create/current change requests and forwards only the stable idempotency header',async()=>{jest.spyOn(global,'fetch').mockImplementation(async()=>new Response('{}',{status:200}));const cookie=Buffer.from(JSON.stringify({credential:TOKEN,expiresAt:EXPIRES_AT}),'utf8').toString('base64url');const hold='66666666-6666-4666-8666-666666666666';const created=await ownerWebBridge(request(`v1/owner/bookings/${hold}/change-requests`,{method:'POST',headers:{cookie:`__Host-vethelp_owner_session=${cookie}`,'idempotency-key':'88888888-8888-4888-8888-888888888888','x-internal-owner':'forbidden'},body:'{"requestType":"CANCEL"'}),['v1','owner','bookings',hold,'change-requests']);expect(created.status).toBe(200);let [url,init]=(global.fetch as jest.Mock).mock.calls[0];expect(String(url)).toBe(`http://backend.test/v1/owner/bookings/${hold}/change-requests`);expect(init.headers.authorization).toBe(`Bearer ${TOKEN}`);expect(init.headers['idempotency-key']).toBe('88888888-8888-4888-8888-888888888888');expect(init.headers['x-internal-owner']).toBeUndefined();await ownerWebBridge(request(`v1/owner/bookings/${hold}/change-requests/current`,{headers:{cookie:`__Host-vethelp_owner_session=${cookie}`}}),['v1','owner','bookings',hold,'change-requests','current']);[url,init]=(global.fetch as jest.Mock).mock.calls[1];expect(String(url)).toBe(`http://backend.test/v1/owner/bookings/${hold}/change-requests/current`);expect(init.method).toBe('GET');});
  it('allowlists only the bounded Owner reallocation create/read/accept routes',async()=>{jest.spyOn(global,'fetch').mockImplementation(async()=>new Response('{}',{status:200}));const cookie=Buffer.from(JSON.stringify({credential:TOKEN,expiresAt:EXPIRES_AT}),'utf8').toString('base64url');const requestId='77777777-7777-4777-8777-777777777771',caseId='88888888-8888-4888-8888-888888888881';await ownerWebBridge(request(`v1/owner/booking-change-requests/${requestId}/reallocation`,{method:'POST',headers:{cookie:`__Host-vethelp_owner_session=${cookie}`,'idempotency-key':'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'},body:'{}'}),['v1','owner','booking-change-requests',requestId,'reallocation']);let[url,init]=(global.fetch as jest.Mock).mock.calls[0];expect(String(url)).toBe(`http://backend.test/v1/owner/booking-change-requests/${requestId}/reallocation`);expect(init.headers['idempotency-key']).toBe('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa');await ownerWebBridge(request(`v1/owner/reallocation-cases/${caseId}`,{headers:{cookie:`__Host-vethelp_owner_session=${cookie}`}}),['v1','owner','reallocation-cases',caseId]);[url,init]=(global.fetch as jest.Mock).mock.calls[1];expect(String(url)).toBe(`http://backend.test/v1/owner/reallocation-cases/${caseId}`);expect(init.method).toBe('GET');expect(init.headers.authorization).toBe(`Bearer ${TOKEN}`);await ownerWebBridge(request(`v1/owner/reallocation-cases/${caseId}/accept`,{method:'POST',headers:{cookie:`__Host-vethelp_owner_session=${cookie}`,'idempotency-key':'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb'},body:'{}'}),['v1','owner','reallocation-cases',caseId,'accept']);[url,init]=(global.fetch as jest.Mock).mock.calls[2];expect(String(url)).toBe(`http://backend.test/v1/owner/reallocation-cases/${caseId}/accept`);expect(init.method).toBe('POST');expect(init.headers['idempotency-key']).toBe('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb');});
