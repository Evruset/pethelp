...
const sameSelector=(left:SpecialistDiscoveryOption|null,right:SpecialistDiscoveryOption)=>{
  ...
  return left.kind==='SERVICE'&&right.kind==='SERVICE'&&left.serviceId===right.serviceId;
};
...
export function SpecialistDiscoveryScreen({onClose,onContinue}:{onClose():void;onContinue(value:SpecialistDiscoverySelection):void}){
  ...
  const results=useQuery({queryKey:['owner',session?.cacheScope,'specialist-discovery',selector?.kind,selector?.kind==='SPECIALTY'?selector.specialtyId:selector?.serviceId],enabled:Boolean(session&&selector),queryFn:({signal})=>specialistDiscoveryApi.search(session!.opaqueCredential,selector!,signal)});
