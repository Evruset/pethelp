...
import { PetJourneyProvider, continuedPetForScope, petSelectionScope, usePetJourney } from './PetJourneyProvider';
import type { PetApi } from './pet-api';

const A={opaqueCredential:`vh_${'a'.repeat(64)}`,cacheScope:'11111111-1111-4111-8111-111111111111',expiresAtEpochMs:Date.now()+60000};
...
beforeAll(()=>{Object.defineProperty(globalThis,'crypto',{configurable:true,value:{randomUUID:()=> '44444444-4444-4444-8444-444444444444'}});});
...
it('auto-selects one pet, continues with exactly that id, and cancel clears only journey state',async()=>{const api={list:jest.fn().mockResolvedValue([ONE]),create:jest.fn()} as jest.Mocked<PetApi>;const view=await setup(api);await act(async()=>fireEvent.press(view.getByText('start')));await waitFor(()=>expect(view.getByText(`selected:${ONE.petId}`)).toBeTruthy());await act(async()=>fireEvent.press(view.getByText('continue')));expect(view.getByText(`continued:${ONE.petId}`)).toBeTruthy();await act(async()=>fireEvent.press(view.getByText('cancel')));expect(view.getByText('active:false')).toBeTruthy();expect(view.getByText('continued:null')).toBeTruthy();expect(view.getByText(`selected:${ONE.petId}`)).toBeTruthy();});
...
it('changes selection ownership for a new credential even when the Owner is the same',()=>{const first=petSelectionScope(A);const second=petSelectionScope({...A,opaqueCredential:`vh_${'b'.repeat(64)}`});expect(first).not.toBe(second);expect(continuedPetForScope({scope:first!,petId:ONE.petId},'authenticated',second)).toBeNull();});
