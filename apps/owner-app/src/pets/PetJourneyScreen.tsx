...
export function PetJourneyScreen({
  ...
  const desktop = width >= t.layout.ownerV50DesktopMinWidth;
  ...
  return (
    <OwnerAppFrame wide>
      ...
      >
        <View style={{ borderWidth: 1, borderColor: t.ownerHome.border, borderRadius: 18, backgroundColor: t.ownerHome.surface, ...t.shadow.card }}>
          <View style={{ padding: desktop ? 20 : 16, justifyContent: 'center', gap: 5 }}>
              ...
              <View style={{ padding: desktop ? 18 : 14, gap: 10, borderWidth: 1, borderColor: t.ownerHome.border, borderRadius: 16, backgroundColor: t.ownerHome.surface }}>
                ...
                <View accessibilityRole="radiogroup" accessibilityLabel="Вид питомца" style={{ flexDirection: desktop ? 'row' : 'column', gap: 8 }}>
                  {(['DOG', 'CAT', 'OTHER'] as PetSpecies[]).map((value) => (
                    ...
                    >
                      <Text style={{ ...t.typography.label, color: t.ownerHome.ink }}>{speciesLabel(value)}</Text>
                    </Pressable>
                  ))}
                </View>
