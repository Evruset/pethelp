...
import { ClinicPatientAdministrativeReferenceDto } from './dto/update-clinic-patient-administrative-reference.dto';

type IdempotencyRow = { status: string; response_status: number | null; response_body: unknown; request_fingerprint: string | null };
type AssociationRow = { id: string };
type ProfileRow = {
  alias: string | null; administrative_reference: string | null;
  aggregate_version: number; updated_at: Date;
};
type PgError = { code?: string; constraint?: string };
const COLLISION_CONSTRAINT = 'clinic_patient_local_profiles_reference_location_key';
const unavailable = (): never => {
  throw new NotFoundException({ code: 'PATIENT_RESOURCE_UNAVAILABLE', message: 'Patient resource unavailable' });
};

@Injectable()
export class ClinicPatientAdministrativeReferenceService {
  ...
  async update(input: {
    ...
    administrativeReference: string | null; comparisonKey: string | null;
    ...
    const scope = ['clinic-patient-administrative-reference-v1', input.employee.sub,
      input.clinicId, input.locationId, input.patientId].join(':');
    const fingerprint = createHash('sha256').update(JSON.stringify({
      administrativeReference: input.administrativeReference,
      operation: input.administrativeReference === null ? 'CLEAR' : 'SET',
    })).digest('hex');
    try {
      return await this.database.withTransaction(async (client) => {
        ...
        try {
          ...
          await this.clinicAccess.assertPatientLocalProfileUpdateAccess(client, input.employee, input.clinicId, input.locationId);
        } catch (error) {
        ...
        const current = await client.query<ProfileRow>(`
          SELECT alias,administrative_reference,aggregate_version,updated_at
        ...
        const previousVersion = previous?.aggregate_version ?? 0;
        ...
        const changed = previous
          ? await client.query<ProfileRow>(`
              ...
              SET administrative_reference=$4,administrative_reference_key=$5,
                  aggregate_version=aggregate_version+1,updated_at=clock_timestamp()
              WHERE clinic_id=$1::uuid AND clinic_location_id=$2::uuid AND patient_id=$3::uuid
                AND aggregate_version=$6
              RETURNING alias,administrative_reference,aggregate_version,updated_at
            `, [input.clinicId, input.locationId, input.patientId,
              input.administrativeReference, input.comparisonKey, previousVersion])
          : await client.query<ProfileRow>(`
              INSERT INTO clinic_schema.clinic_patient_local_profiles
                (clinic_id,clinic_location_id,patient_id,alias,administrative_reference,
                 administrative_reference_key,aggregate_version)
              VALUES ($1::uuid,$2::uuid,$3::uuid,NULL,$4,$5,1)
              RETURNING alias,administrative_reference,aggregate_version,updated_at
            `, [input.clinicId, input.locationId, input.patientId,
              input.administrativeReference, input.comparisonKey]);
        const row = changed.rows[0];
        if (!row) throw DomainErrors.patientVersionStale();
        const result: ClinicPatientAdministrativeReferenceDto = {
          patientId: input.patientId, clinicId: input.clinicId, locationId: input.locationId,
          alias: row.alias, administrativeReference: row.administrative_reference,
          aggregateVersion: row.aggregate_version, updatedAt: row.updated_at.toISOString(),
        };
        const operation = input.administrativeReference === null
          ? 'CLEAR' : previous?.administrative_reference === null || !previous ? 'SET' : 'REPLACE';
        await this.writeAuditAndOutbox(client, input, association.id, operation, previousVersion, row.aggregate_version);
        await this.completeIdempotency(client, scope, input.idempotencyKey, result);
        return result;
      });
    } catch (error) {
      const pg = error as PgError;
      if (pg.code === '23505' && pg.constraint === COLLISION_CONSTRAINT) {
        throw new DomainException(HttpStatus.CONFLICT, 'ADMINISTRATIVE_REFERENCE_ALREADY_IN_USE',
          'Administrative reference is already in use');
      }
      throw error;
    }
  }
  ...
  private async visibleAssociation(client: PoolClient, input: {
    clinicId: string; locationId: string; patientId: string;
  }, policyVersion: string, now: Date): Promise<AssociationRow | undefined> {
  ...
  private async acquireIdempotency(client: PoolClient, scope: string, key: string, fingerprint: string) {
    ...
    const existing = await client.query<IdempotencyRow>(`
      ...
      FROM booking_schema.idempotency_records WHERE scope=$1 AND idempotency_key=$2::uuid FOR UPDATE
    `, [scope, key]);
    ...
    return row.response_body as ClinicPatientAdministrativeReferenceDto;
  }

  private async completeIdempotency(
    client: PoolClient, scope: string, key: string, result: ClinicPatientAdministrativeReferenceDto,
  ): Promise<void> {
    await client.query(`
  ...
  private async writeAuditAndOutbox(
    client: PoolClient,
    input: { clinicId: string; locationId: string; patientId: string; employee: JwtPayload;
      idempotencyKey: string; correlationId: string },
    associationId: string, operation: 'SET' | 'REPLACE' | 'CLEAR',
    previousVersion: number, newVersion: number,
  ): Promise<void> {
    const payload = {
      clinicId: input.clinicId, locationId: input.locationId, patientId: input.patientId, associationId,
      idempotencyKey: input.idempotencyKey, previousVersion, newVersion,
      changedFields: ['administrativeReference'], operation,
    };
    await client.query(`
      INSERT INTO audit_schema.audit_log
        (actor_type,actor_id,action,aggregate_type,aggregate_id,correlation_id,payload_json)
      VALUES ('CLINIC_EMPLOYEE',$1,'clinic.patient-administrative-reference.updated',
    ...
    await client.query(`
      ...
      VALUES ('clinic.patient-administrative-reference.updated.v1','clinic-patient-local-profile',
        $1::uuid,$2::uuid,'ClinicPatientLocalProfile',$3::uuid,$4,$5::jsonb,$6)
    `, [input.correlationId, input.idempotencyKey, associationId, newVersion, JSON.stringify(payload),
      `clinic.patient-administrative-reference.updated.v1:${associationId}:${newVersion}`]);
  }
