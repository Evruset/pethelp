...
export class PublicCatalogService {
  ...
  async readOwnerSpecialistDiscoveryOptions() {
    const result = await this.database.query<OwnerSpecialistOptionRow>(`
      WITH server_time AS (SELECT clock_timestamp() AS value), eligible AS (
        SELECT DISTINCT specialty.id AS specialty_id, specialty.name AS specialty_name,
          upper(service.code) AS service_code, service.display_name AS service_name
        ...
        SELECT service_code, MIN(service_name) AS option_name
        FROM eligible GROUP BY service_code
        ORDER BY MIN(service_name), service_code LIMIT 100
      )
      ...
      SELECT server_time.value AS server_now, 'SERVICE'::text AS option_kind,
        NULL::text AS option_id, service_code AS option_code, option_name
    ...
    const services = new Map<string, { serviceCode: string; name: string }>();
    for (const row of result.rows) {
      ...
      if (row.option_kind === 'SERVICE' && row.option_code) services.set(row.option_code, { serviceCode: row.option_code, name: row.option_name });
    }
    return {
      ...
      services: [...services.values()].sort((a,b)=>a.name.localeCompare(b.name,'ru')||a.serviceCode.localeCompare(b.serviceCode)).slice(0,100),
    };
  }

  async readOwnerSpecialistDiscovery(input: { specialtyId?: string; serviceCode?: string; limit: number }) {
    ...
    `, [input.specialtyId ?? null, input.serviceCode ?? null, input.limit]);
