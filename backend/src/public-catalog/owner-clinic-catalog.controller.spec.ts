const mockFeatureFlags={OWNER_R2E_SPECIALIST_PROJECTION:true};
jest.mock('../config/feature-flags.config',()=>({featureFlags:mockFeatureFlags}));
...
describe('OwnerClinicCatalogController',()=>{
  ...
  it('normalizes a bounded specialist selector and rejects an empty selection',async()=>{
    ...
    expect(readOwnerSpecialistDiscovery).toHaveBeenCalledWith({specialtyId:undefined,serviceCode:'PRIMARY_EXAM',serviceId:undefined,limit:7});
    const serviceId='33333333-3333-4333-8333-333333333333';
    await controller.specialistDiscovery({serviceId,limit:7});
    expect(readOwnerSpecialistDiscovery).toHaveBeenLastCalledWith({specialtyId:undefined,serviceCode:undefined,serviceId,limit:7});
  ...
  it('keeps specialist discovery unavailable unless explicitly enabled',async()=>{mockFeatureFlags.OWNER_R2E_SPECIALIST_PROJECTION=false;const controller=new OwnerClinicCatalogController({} as PublicCatalogService);await expect(controller.specialistDiscoveryOptions()).rejects.toMatchObject({status:404,response:{code:'OWNER_SPECIALIST_PROJECTION_DISABLED'}});mockFeatureFlags.OWNER_R2E_SPECIALIST_PROJECTION=true;});
});
