...
const ROLE_CAPABILITIES: Readonly<Partial<Record<Role, readonly Capability[]>>> = {
  ...
  [Role.CLINIC_RECEPTIONIST]: [Capability.BOOKING_QUEUE_READ, Capability.APPOINTMENT_REGISTRY_READ, Capability.QUALITY_READ, Capability.SCHEDULE_READ, Capability.BOOKING_REPLAY_READ, Capability.BOOKING_HOLD_READ],
  [Role.CLINIC_ADMIN]: [Capability.BOOKING_QUEUE_READ, Capability.APPOINTMENT_REGISTRY_READ, Capability.QUALITY_READ, Capability.SCHEDULE_READ, Capability.BOOKING_REPLAY_READ, Capability.BOOKING_HOLD_READ],
...
export type CapabilityResource = {
  aggregateType: 'booking.queue' | 'appointment.registry' | 'booking.hold' | 'booking.hold.replay' | 'clinical.visit' | 'clinical.visit.workspace' | 'quality.dashboard' | 'schedule.slots';
  clinicId?: string;
  locationId: string;
} | {
