...
export class MisCommandDispatcherService {
  ...
  /**
   * The external call is intentionally outside a database transaction.
   * The only transactions in this class are short state commits protected by
   * lock_timeout and statement_timeout budgets.
   */
  async dispatchReservation(payload: MisReservationRequestedPayload): Promise<void> {
    ...
    if (context.state !== 'MIS_RESERVATION_PENDING') {
    ...
    for (let attempt = 0; attempt <= this.retryDelaysMs.length; attempt += 1) {
      try {
        ...
        if (result.status === 'SUCCESS') {
          await this.commitSuccess(payload, result);
        } else {
          await this.commitFailure(payload, result.rawError ?? 'MIS reservation was rejected');
        }
        return;
      } catch (error) {
  ...
  private async commitSuccess(
    ...
    result: MisReservationResult,
  ): Promise<void> {
    if (!result.externalHoldId) {
      await this.commitFailure(payload, 'MIS response marked success without external hold id');
      return;
    }
    ...
    await this.database.withTransaction(async (client) => {
      ...
      if (locked.hold_state !== 'MIS_RESERVATION_PENDING') return;
      ...
      const updated = await client.query<{ version: number }>(`
        ...
        SET state = 'MIS_HELD',
            ...
            expires_at = CASE
              ...
              ELSE LEAST(expires_at, clock_timestamp() + ($3::text || ' minutes')::interval)
            END,
      ...
      await this.writeAudit(client, 'mis.reservation.held', payload, {
        externalHoldId: result.externalHoldId,
        ttlMinutes: result.ttlMinutes,
      });
      await this.writeOutbox(client, 'mis.reservation.held.v1', payload, updated.rows[0].version, {
        externalHoldId: result.externalHoldId,
        ttlMinutes: result.ttlMinutes,
      });
    });
  ...
  private async commitFailure(
    ...
    await this.database.withTransaction(async (client) => {
      ...
      `, [locked.hold_slot_id]);
      if (!slot.rows[0]) {
        throw new Error(`Cannot compensate hold ${payload.holdId}: held_count is already zero`);
      }

      await this.writeAudit(client, 'mis.reservation.failed', payload, { rawError: rawError.slice(0, 1000) });
      await this.writeOutbox(client, 'mis.reservation.failed.v1', payload, hold.rows[0].version, {
        rawError: rawError.slice(0, 1000),
      });
    });
  }
  ...
  private async lockReservation(client: PoolClient, holdId: string): Promise<LockedReservation | undefined> {
    const result = await client.query<LockedReservation>(`
      SELECT h.id AS hold_id,
             ...
             h.slot_id AS hold_slot_id,
             s.id AS slot_id,
             s.held_count AS slot_held_count
      FROM booking_schema.booking_holds h
      JOIN clinic_schema.appointment_slots s ON s.id = h.slot_id
      WHERE h.id = $1::uuid
      FOR UPDATE OF h, s
  ...
  private errorMessage(error: unknown): string {
    if (error instanceof MisConfigurationError || error instanceof MisNetworkError || error instanceof Error) {
