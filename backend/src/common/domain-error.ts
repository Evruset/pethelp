...
export const DomainErrors = {
  ...
  bookingStateConflict: () => new DomainException(HttpStatus.CONFLICT, 'BOOKING_STATE_CONFLICT', 'Booking state or version is stale'),
  ...
  idempotencyConflict: () => new DomainException(HttpStatus.CONFLICT, 'IDEMPOTENCY_CONFLICT', 'Idempotency key was already used for another request'),
  ...
  alternativeSlotUnavailable: () => new DomainException(HttpStatus.CONFLICT, 'ALTERNATIVE_SLOT_UNAVAILABLE', 'Alternative slot is no longer available'),
  alternativeSlotIncompatible: () => new DomainException(HttpStatus.CONFLICT, 'ALTERNATIVE_SLOT_INCOMPATIBLE', 'Alternative slot is incompatible with the booking'),
