...
describe('W7-A clinical foundation migration', () => {
  it('proves lineage, lifecycle, immutability, projection coherence and populated DOWN fail-closed', async () => {
    ...
    const previous=join(process.cwd(),'migrations/node-pg/1719530000000_add_owner_otp_anti_fraud.js');const previousChecksum=sha(previous);
    await admin.connect();
    try {
      ...
      const visit2=(await client.query<{id:string}>(`INSERT INTO clinical_schema.visits(appointment_id,booking_hold_id,owner_id,pet_id,clinic_id,location_id,slot_id,completed_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,[I.otherAppointment,I.otherHold,I.otherOwner,I.otherPet,I.otherClinic,I.otherLocation,I.otherSlot,I.vet])).rows[0];
      const draft2=(await client.query<{id:string}>(`INSERT INTO clinical_schema.visit_results(visit_id,owner_id,pet_id,clinic_id,location_id,author_id,clinical_summary,idempotency_key) VALUES($1,$2,$3,$4,$5,$6,'Second draft',$7) RETURNING id`,[visit2.id,I.otherOwner,I.otherPet,I.otherClinic,I.otherLocation,I.vet,randomUUID()])).rows[0];
      await expect(client.query(`INSERT INTO clinical_schema.visit_result_amendments(result_id,visit_id,owner_id,pet_id,clinic_id,location_id,author_id,amendment_content,idempotency_key) VALUES($1,$2,$3,$4,$5,$6,$7,'Not allowed',$8)`,[draft2.id,visit2.id,I.otherOwner,I.otherPet,I.otherClinic,I.otherLocation,I.vet,randomUUID()])).rejects.toMatchObject({code:'23514'});
