...
describe('Wave 3 DoctorShift generated inventory (real PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    await database.query(`INSERT INTO catalog_schema.specialty_services(service_id,specialty_id,created_by) VALUES($1,$2,$3)`, [ids.service, ids.specialty, ids.employee]);
    await database.query(`INSERT INTO catalog_schema.specialty_services(service_id,specialty_id,created_by) VALUES($1,$2,$3)`, [ids.service, ids.specialty, ids.employee]);
  ...
  afterAll(async () => {
    ...
    await database.query(`DELETE FROM catalog_schema.specialty_services WHERE service_id=$1`, [ids.service]);
    await database.query(`DELETE FROM catalog_schema.specialty_services WHERE service_id=$1`, [ids.service]);
  ...
  it('projects only eligible specialist-first doctors with live published inventory and stable identities',async()=>{
    ...
    expect(options.services).toContainEqual({serviceId:ids.service,specialtyId:ids.specialty,serviceCode:serviceRow.code.toUpperCase(),name:'Wave3 Service'});
    ...
    await database.query(`UPDATE catalog_schema.specialty_services SET active=false WHERE service_id=$1`,[ids.service]);
    expect((await catalog.readOwnerSpecialistDiscovery({serviceId:ids.service,limit:25})).doctors).toEqual([]);
    await database.query(`UPDATE catalog_schema.specialty_services SET active=true WHERE service_id=$1`,[ids.service]);
    await database.query(`UPDATE catalog_schema.specialty_services SET active=false WHERE service_id=$1`,[ids.service]);
    expect((await catalog.readOwnerSpecialistDiscovery({serviceId:ids.service,limit:25})).doctors).toEqual([]);
    expect((await catalog.readOwnerSpecialistDiscoveryOptions()).services).not.toEqual(expect.arrayContaining([expect.objectContaining({serviceId:ids.service})]));
    await database.query(`UPDATE catalog_schema.specialty_services SET active=true WHERE service_id=$1`,[ids.service]);
