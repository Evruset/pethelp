...
describe('Marketplace Alpha alternative slots', () => {
  ...
  it('keeps the original slot held until owner accepts an alternative', async () => {
    ...
    const proposal = await trace.run({ correlationId: randomUUID(), userId: fixture.employeeId }, () =>
      service.proposeAlternativeSlot(fixture.holdId, fixture.alternativeSlotId, employee),
    );

    expect(proposal.state).toBe('ALTERNATIVE_PENDING');
    expect(proposal.sourceSlotId).toBe(fixture.sourceSlotId);
    expect(proposal.alternativeSlotId).toBe(fixture.alternativeSlotId);

    const beforeAccept = await database.query<{ source_held: number; alternative_held: number; appointments: string }>(`
      SELECT
        ...
        (SELECT COUNT(*)::text FROM booking_schema.appointments WHERE hold_id = $3::uuid) AS appointments
    `, [fixture.sourceSlotId, fixture.alternativeSlotId, fixture.holdId]);
    expect(beforeAccept.rows[0]).toMatchObject({ source_held: 1, alternative_held: 1, appointments: '0' });
    ...
    expect(accepted.slotId).toBe(fixture.alternativeSlotId);
    ...
    const afterAccept = await database.query<{
      ...
      SELECT
        (SELECT held_count FROM clinic_schema.appointment_slots WHERE id = $1::uuid) AS source_held,
        (SELECT status FROM clinic_schema.appointment_slots WHERE id = $1::uuid) AS source_status,
        (SELECT held_count FROM clinic_schema.appointment_slots WHERE id = $2::uuid) AS alternative_held,
        (SELECT booked_count FROM clinic_schema.appointment_slots WHERE id = $2::uuid) AS alternative_booked,
        (SELECT status FROM clinic_schema.appointment_slots WHERE id = $2::uuid) AS alternative_status,
        (SELECT slot_id::text FROM booking_schema.appointments WHERE hold_id = $3::uuid) AS appointment_slot_id
    `, [fixture.sourceSlotId, fixture.alternativeSlotId, fixture.holdId]);
    ...
    expect(afterAccept.rows[0]).toMatchObject({
      ...
      alternative_held: 0,
      alternative_booked: 1,
      alternative_status: 'BOOKED',
      appointment_slot_id: fixture.alternativeSlotId,
    });
  });
});
...
async function createFixture(database: DatabaseService): Promise<{
  ...
  const source = await database.query<{ id: string }>(`
    INSERT INTO clinic_schema.appointment_slots (
      clinic_location_id, service_id, starts_at, ends_at, capacity,
      held_count, status, integration_mode
    ) VALUES (
      $1::uuid, $2::uuid, clock_timestamp() + interval '2 hours', clock_timestamp() + interval '150 minutes', 1,
      1, 'LOCKED_BY_HOLD', 'LEVEL_C'
    ) RETURNING id
  `, [location.rows[0].id, clinicService.rows[0].id]);
  const alternative = await database.query<{ id: string }>(`
    INSERT INTO clinic_schema.appointment_slots (
      clinic_location_id, service_id, starts_at, ends_at, capacity, status, integration_mode
    ) VALUES (
      $1::uuid, $2::uuid, clock_timestamp() + interval '3 hours', clock_timestamp() + interval '210 minutes', 1, 'AVAILABLE', 'LEVEL_C'
    ) RETURNING id
  `, [location.rows[0].id, clinicService.rows[0].id]);
