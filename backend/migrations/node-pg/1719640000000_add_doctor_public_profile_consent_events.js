  ...
  pgm.sql(`
    CREATE TABLE catalog_schema.doctor_public_profile_consent_events (
      ...
      CONSTRAINT doctor_public_profile_consent_events_actor_location_fkey
        ...
        REFERENCES clinic_schema.employee_location_memberships (employee_id, clinic_location_id)
        ON DELETE RESTRICT,
    ...
    BEGIN
      ...
      WHERE doctor.id = NEW.doctor_id
        AND doctor.clinic_location_id = NEW.clinic_location_id
      FOR UPDATE;
      ...
      IF actor_authorized IS DISTINCT FROM true THEN
        RAISE EXCEPTION 'PUBLIC_DOCTOR_PROFILE_CONSENT_ACTOR_NOT_AUTHORIZED'
          USING ERRCODE = '23514';
      END IF;
      IF NEW.occurred_at > clock_timestamp() OR NEW.created_at < NEW.occurred_at THEN
        RAISE EXCEPTION 'PUBLIC_DOCTOR_PROFILE_CONSENT_TIME_INVALID'
          USING ERRCODE = '23514';
      END IF;
      ...
      WHERE event.doctor_id = NEW.doctor_id
        AND event.clinic_location_id = NEW.clinic_location_id
      ORDER BY event.occurred_at DESC, event.id DESC
      LIMIT 1;

      IF previous_occurred_at IS NOT NULL AND NEW.occurred_at <= previous_occurred_at THEN
        RAISE EXCEPTION 'PUBLIC_DOCTOR_PROFILE_CONSENT_TIME_NOT_MONOTONIC'
          USING ERRCODE = '23514';
      END IF;

      IF NEW.event_type = 'CONSENT_GRANTED' AND previous_event = 'CONSENT_GRANTED' THEN
        RAISE EXCEPTION 'PUBLIC_DOCTOR_PROFILE_CONSENT_ALREADY_GRANTED'
          USING ERRCODE = '23514';
      END IF;
      IF NEW.event_type = 'CONSENT_REVOKED'
         AND previous_event IS DISTINCT FROM 'CONSENT_GRANTED' THEN
        RAISE EXCEPTION 'PUBLIC_DOCTOR_PROFILE_CONSENT_NOT_GRANTED'
          USING ERRCODE = '23514';
      END IF;
      RETURN NEW;
    END $$;
    ...
    RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
      RAISE EXCEPTION 'PUBLIC_DOCTOR_PROFILE_CONSENT_AUDIT_IMMUTABLE'
        USING ERRCODE = '23514';
    END $$;
