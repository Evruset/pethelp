...
async function main(): Promise<void> {
  ...
  try {
    ...
    await client.query('INSERT INTO identity_schema.users (id) VALUES ($1::uuid) ON CONFLICT (id) DO NOTHING', [employeeId]);
    ...
    if (!row) throw new Error('VetHelp Pilot clinic location was not found. Run the seed profile first.');
    await client.query(`
    ...
    console.log(JSON.stringify({ employeeId, clinicId: row.clinic_id, locationId: row.location_id, role: 'CLINIC_RECEPTIONIST' }));
  } catch (error) {
