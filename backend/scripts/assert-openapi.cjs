...
async function main() {
  ...
  required(catalogItem?.required?.sort().join() === 'address,clinicId,locationId,name,phone', 'Owner clinic catalog projection required fields are incomplete');
  required(Object.keys(catalogItem?.properties ?? {}).sort().join() === 'address,clinicId,locationId,name,phone', 'Owner clinic catalog exposes non-MVP fields');
  ...
  required(serviceItem?.additionalProperties === false && serviceItem.required?.sort().join() === 'name,price,serviceId', 'Owner clinic service item must be closed');
  required(Object.keys(serviceItem?.properties ?? {}).sort().join() === 'name,price,serviceId' && serviceItem.properties.serviceId?.format === 'uuid' && serviceItem.properties.price?.$ref === '#/components/schemas/OwnerClinicServicePriceDto', 'Owner clinic service fields must be exact and typed');
  required(servicePrice?.additionalProperties === false && servicePrice.required?.sort().join() === 'amount,currency,kind' && servicePrice.properties.kind?.enum?.join() === 'INFORMATIONAL', 'Owner clinic service price must be informational-only');
  ...
  required(availability?.additionalProperties===false&&availability.required?.sort().join()==='clinicName,horizonEndsAt,observedAt,serviceName,slots,timezone'&&Object.keys(availability.properties??{}).sort().join()==='clinicName,horizonEndsAt,observedAt,serviceName,slots,timezone','Owner availability envelope must be exact and closed');
