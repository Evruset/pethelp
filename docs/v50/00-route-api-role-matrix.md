...
| `clinic-patients` | scoped Portal registry at `/clinics/:clinicId/locations/:locationId/patients`; patient detail remains separate | `ClinicPatientsRegistry` plus scoped BFF | canonical backend exact-location GET over association revisions | **end-to-end read implemented; detail/mutations absent** |
| `clinic-patient` | future exact-location administrative detail; clinical record remains separate | absent; navigation intentionally disabled | contracted future `GET /v1/clinic/:clinicId/locations/:locationId/patients/:patientId`, where patientId is Registry pet UUID plus current visible association | **contract ready / backend and Portal absent** |
...
| Read patient administrative detail | contracted future exact-location GET and Portal page; not implemented | reuse `patient.admin.read`; receptionist/admin or multi-role with effective capability, exact active membership/current association; veterinarian-only denied | safe pet identity, nullable owner display, relationship timestamps and bounded appointment summaries; current revoke overrides stale link | no admin read audit added; clinical per-view audit requires separate policy | `READY_FOR_ADMIN_DETAIL_BACKEND` |
