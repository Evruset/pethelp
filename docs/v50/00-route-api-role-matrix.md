...
| `clinic-patient` | exact-location administrative detail; clinical record remains separate | absent; navigation intentionally disabled | implemented `GET /v1/clinic/:clinicId/locations/:locationId/patients/:patientId`, where patientId is Registry pet UUID plus current visible association | **backend implemented / Portal absent** |
...
| Read patient administrative detail | exact-location backend GET implemented; Portal page absent | reuse `patient.admin.read`; receptionist/admin or multi-role with effective capability, exact active membership/current association; veterinarian-only denied | safe pet identity, nullable owner display, relationship timestamps and bounded appointment summaries; current revoke overrides stale link | no admin read audit added; clinical per-view audit requires separate policy | `PATIENT_ADMINISTRATIVE_DETAIL_BACKEND_IMPLEMENTED` |
