...
| --- | --- | --- | --- | --- |
| `clinic-workspace` | capability-filtered exact-location operational home; unproven classifications fail closed | canonical future route `/clinics/:clinicId/locations/:locationId`; V50 shell exists, page runtime absent | **CONTRACT_READY / 05A-R1 safe minimum**: future `GET /v1/clinic/:clinicId/locations/:locationId/workspace-home` uses one common exact-scope authority gate; Queue/Appointments may return bounded facts, while Schedule/Veterinarian/Quality are no-facts `NOT_CONFIGURED` when authorized; no Portal route implementation | **contract ready, runtime absent**; two-summary query budget, no full-list composition, no Schedule/Veterinarian/Quality operational SQL; missing capability remains `NOT_AUTHORIZED` |
