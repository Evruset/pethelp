...
| --- | --- | --- | --- | --- |
| `clinic-workspace` | capability-filtered exact-location operational home; doctor view must not claim personal assignment without assignment authority | canonical future route `/clinics/:clinicId/locations/:locationId`; V50 shell exists, page runtime absent | **CONTRACT_READY**: one future `GET /v1/clinic/:clinicId/locations/:locationId/workspace-home` bounded backend projection via one same-origin Portal BFF; no full-list composition | **contract ready, runtime absent**; Queue/Schedule/Appointments/Quality use existing capabilities; 05B veterinarian section remains `NOT_CONFIGURED`, fact/action/identifier-free until assignment authority exists |
