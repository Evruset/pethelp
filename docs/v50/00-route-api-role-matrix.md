...
| `clinic-patients` | future scoped administrative registry at `/clinics/:clinicId/locations/:locationId/patients`; patient detail remains separate | отсутствует | association schema/lifecycle implemented but producer not wired; future GET remains blocked | **schema/lifecycle implemented / producer prerequisite** |
...
| Read patient administrative registry | contract only: future exact-location patients GET over active hybrid association | future `patient.admin.read`; receptionist/admin mapping proposed, exact active membership; purpose-specific consent; no clinical category | none | association/consent lifecycle audit; no medical read audit for allowlist | association schema/lifecycle implemented / appointment producer prerequisite |
