...
| `clinic-patients` | future scoped administrative registry at `/clinics/:clinicId/locations/:locationId/patients`; patient detail remains separate | отсутствует | hybrid association/consent decision complete; future GET remains blocked by association schema contract | **contract/decision complete / schema prerequisite** |
...
| Read patient administrative registry | contract only: future exact-location patients GET over active hybrid association | future `patient.admin.read`; receptionist/admin mapping proposed, exact active membership; purpose-specific consent; no clinical category | none | association/consent lifecycle audit; no medical read audit for allowlist | contract/decision complete / association schema prerequisite |
