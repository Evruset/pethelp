...
| Appointment administrative detail | contract: future `GET /v1/clinic/:clinicId/locations/:locationId/appointments/:appointmentId`; implementation absent | reuse `appointment.registry.read`; exact clinic/location capability authority | read-only; `availableActions: []` until separate command contracts | no read audit/outbox | contract complete / implementation missing |
