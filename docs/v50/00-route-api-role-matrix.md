...
| `clinic-patients` | future scoped Portal registry at `/clinics/:clinicId/locations/:locationId/patients`; patient detail remains separate | отсутствует | canonical backend exact-location GET implemented over association revisions | **backend read implemented / Portal missing** |
...
| Read patient administrative registry | backend `GET /v1/clinic/:clinicId/locations/:locationId/patients`; Portal absent | `patient.admin.read`; receptionist/admin, exact JWT clinic/location and active non-revoked membership; purpose-specific consent; veterinarian not inferred | revision-snapshot read over current policy-valid association; manual-confirm producer writes association/receipt/revision/outbox atomically | read-only; no medical read audit for allowlist | backend read implemented and focused PostgreSQL/OpenAPI validated under default-off patients flag |
