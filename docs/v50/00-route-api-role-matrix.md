...
| `clinic-appointments` | admin registry/detail/actions; doctor personal shift only | queue route remains separate; Portal registry absent | `GET /v1/clinic/:clinicId/locations/:locationId/appointments` implements the bounded location-scoped cursor list with production-scale ordered query indexes; detail/check-in/reschedule APIs absent | **backend read/index path implemented; Portal missing** |
