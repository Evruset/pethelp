...
| --- | --- | --- | --- | --- |
| `clinic-workspace` | capability-filtered exact-location operational home; unproven classifications fail closed | canonical Portal route `/clinics/:clinicId/locations/:locationId`; V50 shell exists, page runtime absent | **BACKEND_FOUNDATION_IMPLEMENTED / TESTED**: `GET /v1/clinic/:clinicId/locations/:locationId/workspace-home` uses one common exact-scope authority gate; Queue/Appointments return capability-filtered bounded facts, while Schedule/Veterinarian/Quality are no-facts `NOT_CONFIGURED` when authorized | read-only repeatable-read snapshot; maximum two operational summaries, no full-list composition or Schedule/Veterinarian/Quality SQL; missing capability is `NOT_AUTHORIZED`; Portal/BFF is 05C |
