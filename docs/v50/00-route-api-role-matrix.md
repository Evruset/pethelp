...
| Update clinic-local patient alias | contract only; future exact-location `PATCH .../patients/:patientId/local-profile` | new `patient.admin.local-profile.update`; read capability is insufficient | clinic-location association projection; owner pet master unchanged | versioned audit + durable outbox required | `04D CONTRACT_COMPLETE / 04E IMPLEMENTATION_MISSING` |
