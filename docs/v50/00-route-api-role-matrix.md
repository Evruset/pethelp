...
| `clinic-patients` | future scoped administrative registry at `/clinics/:clinicId/locations/:locationId/patients`; patient detail remains separate | отсутствует | hybrid association/consent/schema contracts complete; future GET remains blocked by association migration | **contract complete / migration prerequisite** |
...
| Read patient administrative registry | contract only: future exact-location patients GET over active hybrid association | future `patient.admin.read`; receptionist/admin mapping proposed, exact active membership; purpose-specific consent; no clinical category | none | association/consent lifecycle audit; no medical read audit for allowlist | contract/schema complete / association migration prerequisite |
