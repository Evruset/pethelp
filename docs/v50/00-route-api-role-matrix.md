...
| Appointment administrative detail | `GET /v1/clinic/:clinicId/locations/:locationId/appointments/:appointmentId`; backend implemented, shared registry flag default-off; Portal/BFF absent | `appointment.registry.read`; admin/reception + exact clinic/location claims and active membership | read-only; `availableActions: []` until separate command contracts | no read audit/outbox | `DETAIL_BACKEND_IMPLEMENTED / PORTAL_MISSING`; focused HTTP 13/13 |
