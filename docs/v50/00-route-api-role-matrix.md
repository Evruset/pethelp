...
| `clinic-patients` | future scoped administrative registry at `/clinics/:clinicId/locations/:locationId/patients`; patient detail remains separate | отсутствует | association schema/lifecycle/manual-confirm producer implemented; future GET remains absent | **write side implemented / backend read-model prerequisite** |
...
| Read patient administrative registry | contract only: future exact-location patients GET over active hybrid association | future `patient.admin.read`; receptionist/admin mapping proposed, exact active membership; purpose-specific consent; no clinical category | manual-confirm producer writes association/receipt/revision/outbox atomically | association/consent lifecycle audit; no medical read audit for allowlist | write side implemented / backend read-model prerequisite |
