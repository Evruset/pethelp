...
| CLN-004 | `prototype-v50/manifest.json#sha256` | Clinic / Appointments | `#clinic-appointments` → scoped `/appointments` | default-off Portal registry plus backend cursor/index path implemented; detail absent; Queue unchanged | admin/reception `appointment.registry.read`; active exact clinic/location authority | scoped Portal BFF → `GET /v1/clinic/:clinicId/locations/:locationId/appointments`; signed snapshot/keyset cursor | registry read-only; existing Queue commands remain separate | L/Ø/E/C; upcoming/history, loading, distinct empty/error, degraded page, unknown status, privacy | D/T/M responsive cards; desktop/mobile evidence | `VETHELP_CLINIC_APPOINTMENTS_REGISTRY`, default off; page and BFF absent on rollback | Portal Chromium enabled 24/24 + rollback 1/1; Node 22 typecheck/build; registry 27/27; query-index 6/6 | MODIFY | END_TO_END_IMPLEMENTED / DETAIL_ACTIONS_MISSING |
