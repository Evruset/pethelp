...
| CLN-005 | `prototype-v50/manifest.json#sha256` | Clinic / Patients | `#clinic-patients` → future `/clinics/:clinicId/locations/:locationId/patients` | canonical administrative registry contract complete; API/UI absent | future `patient.admin.read`; receptionist/admin, exact clinic/location and active membership; clinical access separate | appointment-derived, deduplicated active-pet projection; no owner contacts/medical data | read-only; detail/create/update absent | L/Ø/E/C/degraded/no-results/access-denied contracted | D/T/M/a11y contracted, not implemented | future separate default-off patients flag; not added | future inclusion/privacy/search/cursor/performance/rollback matrix defined | MODIFY | CONTRACT_COMPLETE / IMPLEMENTATION_BLOCKED_BY_RETENTION_CONSENT_DECISION |
