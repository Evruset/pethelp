...
| CLN-005 | `prototype-v50/manifest.json#sha256` | Clinic / Patients | `#clinic-patients` → future `/clinics/:clinicId/locations/:locationId/patients` | association schema, transactional lifecycle and manual-confirm producer implemented; API/UI absent | future `patient.admin.read`; exact clinic/location, active membership and purpose-specific consent; clinical access separate | versioned exact-location appointment-provenance association; temporal registry revisions; no owner contacts/medical data | read-only; detail/create/update/import/manual absent | L/Ø/E/C/degraded/no-results/access-denied contracted | D/T/M/a11y contracted, not implemented | future separate default-off patients flag; missing visibility/search policy fails closed | producer atomicity/dedup/consent-deny suite; backend read model required | MODIFY | WRITE_SIDE_IMPLEMENTED / BACKEND_READ_MODEL_REQUIRED |
