...
| CLN-001 | `prototype-v50/manifest.json#sha256` | Clinic/Vet / Workspace home | `#clinic-workspace` → `/clinics/:clinicId/locations/:locationId` | backend foundation projection implemented; Portal/BFF/page not started | exact active clinic/location membership plus per-section existing capabilities | bounded Queue + Appointments summaries; Schedule/Veterinarian/Quality fail closed as no-facts `NOT_CONFIGURED`; no identifiers | Queue/Appointments read-only action routes only when section capability is allowed | backend partial degradation tested; Portal loading/empty/error/stale/forbidden states not started | D/T/M and accessibility behavior specified, not visually accepted | future default-off `CLINIC_V50_WORKSPACE_HOME`, dependent on `PORTAL_V50_SHELL` | PostgreSQL authority/privacy/concurrency and 10k two-summary performance/OpenAPI matrix pass | BACKEND_FOUNDATION_IMPLEMENTED / TESTED | 05B implemented/tested; PORTAL_NOT_STARTED / VISUAL_NOT_VERIFIED |
