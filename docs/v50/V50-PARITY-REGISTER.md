...
| CLN-005 | `prototype-v50/manifest.json#sha256` | Clinic / Patients | `#clinic-patients` → future `/clinics/:clinicId/locations/:locationId/patients` | association/lifecycle/manual-confirm producer plus canonical backend registry GET implemented; Portal absent | `patient.admin.read`; receptionist/admin, exact clinic/location, active membership and purpose-specific consent; clinical access separate | signed revision snapshot with current privacy override; administrative allowlist; no owner contacts/medical data | read-only; detail/create/update/import/manual absent | backend empty/no-results/error/access-denied validated; Portal states absent | D/T/M/a11y contracted, not implemented | separate default-off patients flag; missing visibility/search policy fails closed | Patients PostgreSQL 8/8, producer/lifecycle/capability 44/44, full-query EXPLAIN over 120 added associations, Node 22/OpenAPI/migrations PASS | MODIFY | BACKEND_READ_IMPLEMENTED / TESTED / PORTAL_MISSING |
