...
| CLN-005 | `prototype-v50/manifest.json#sha256` | Clinic / Patients | `#clinic-patients` → `/clinics/:clinicId/locations/:locationId/patients` | association/lifecycle/manual-confirm producer, backend GET, scoped BFF and responsive read-only Portal registry implemented | `patient.admin.read`; Portal effective exact scope plus backend active membership/consent; clinical access separate | signed revision snapshot with current privacy override; administrative allowlist; no owner contacts/medical data | search/load-more/manual refresh only; detail/create/update/import/manual absent | loading, empty, no-results, ready, degraded, rate, denied and technical states implemented | responsive cards, keyboard, axe, 200% text and desktop/mobile captures PASS | separate default-off patients flag; production search fails closed without shared limiter | Chromium 9/9 + rollback 1/1; backend Patients 8/8; Node 22 Portal typecheck/build PASS | MODIFY | END_TO_END_READ_IMPLEMENTED / PATIENT_DETAIL_AND_MUTATIONS_MISSING |
