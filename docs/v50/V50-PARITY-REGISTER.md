...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| OWN-001 | `prototype-v50/manifest.json#sha256` | Owner / Home | `#home` → `/owner/home` | `OwnerJourneyEntry` → owner home; anonymous Navigator route | `OWNER`; target `owner.home.read` | pets, `GET /v1/owner/appointments` | none | L/Ø/E/O; selected-pet, next-safe-step | D/T/M | MISSING | owner journey widget/integration; `owner_journey_main.dart` | MODIFY | DISCOVERY / none |
...
| OWN-009 | `prototype-v50/manifest.json#sha256` | Owner / Pets | `#pets` → `/owner/pets` | `OwnerPetsPage` | `OWNER`; `pet.own.read/write` | `GET /v1/owner/pets` | `POST /v1/owner/pets` | L/Ø/E/O; active/selected | D/T/M | MISSING | pets widget/API tests | REUSE | DISCOVERY / none |
| OWN-010 | `prototype-v50/manifest.json#sha256` | Owner / Pet profile | `#pet-profile` → `/owner/pets/:petId` | split across `OwnerPetsPage` and `OwnerPetCarePage` | `OWNER`; `pet.own.read/write` | pet detail/care summary/documents | PATCH pet, upload document | L/Ø/E/O; processing/failed document | D/T/M | MISSING | pets/care/document tests | MODIFY | DISCOVERY / route |
| OWN-011 | `prototype-v50/manifest.json#sha256` | Owner / Pet Diary | `#diary` → `/owner/pets/:petId/diary` | `OwnerPetCarePage` | `OWNER`; target `pet.diary.read` | care summary/documents | document upload; reminders missing | L/Ø/E/O; visits/docs/reminders | D/T/M | MISSING | care/document widget/integration | MODIFY | DISCOVERY / reminder contract |
...
| CLN-001 | `prototype-v50/manifest.json#sha256` | Clinic/Vet / Workspace home | `#clinic-workspace` → scoped clinic root | portal `/` informational; no role-specific dashboard | reception/admin/vet by server capabilities | queue/quality/shift projections fragmented | task commands by capability | L/Ø/E/C; SLA/tasks/role mode | D/T/M | `PORTAL_V51_SHELL` only | required dashboard/shift E2E | MISSING | DISCOVERY / read models |
| CLN-002 | `prototype-v50/manifest.json#sha256` | Clinic / Schedule | `#clinic-schedule` → `/clinics/:clinicId/locations/:locationId/schedule` | `ClinicScheduleClient` route exists | admin/reception; `schedule.read/manage` | schedule slots/snapshot | hours, services, staff, resources, periods, slots, import/export | L/Ø/E/C; open/blocked/booked/telemed | D/T/M | shell + read capability flags | schedule capability Playwright/build | REUSE | DISCOVERY / visual |
| CLN-003 | `prototype-v50/manifest.json#sha256` | Vet / Visit workspace | `#clinic-visit` → `/clinics/:clinicId/locations/:locationId/vet/visits/:holdId` | bounded vet list/detail/completion routes exist | clinic veterinarian; `clinical.visit.workspace.read/complete` | vet visit list/detail | complete; full draft/sign/amend absent | L/E/C/T; confirmed, in-visit, completed, immutable/amend | D/T/M | shell flag; additive reads | veterinarian read 6/6, completion 8/8 historical | MODIFY | DISCOVERY / clinical model |
...
When any prototype file changes, regenerate the manifest, rerun inventory verification, diff screen/route/state identities, and update affected evidence before implementation continues. The next selected slice is `V50-SHELL-01`; source confirmation carries no visual acceptance forward.
