...
| CLN-004 | `prototype-v50/manifest.json#sha256` | Clinic / Appointments | `#clinic-appointments` → scoped `/appointments` | queue route/commands; registry list contract defined, production registry/detail absent | admin/reception target `appointment.registry.read`; existing queue authority unchanged | target location-scoped cursor registry; existing queue/hold/replay only | existing confirm/decline/notes/alternative remain Queue-owned; registry contract is read-only | L/Ø/E/C; bounded upcoming/history contract; prototype check-in/done remain future | D/T/M future implementation | future default-off registry rollout; existing queue flags unchanged | contract authority/read-model matrix defined; implementation tests pending | MODIFY | CONTRACT_COMPLETE / IMPLEMENTATION_MISSING |
