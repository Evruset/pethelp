...
| CLN-001 | `prototype-v50/manifest.json#sha256` | Clinic/Vet / Workspace home | `#clinic-workspace` → `/clinics/:clinicId/locations/:locationId` | contract selects one backend capability-filtered projection; runtime read model/page remain absent | exact active clinic/location membership plus per-section existing capabilities | typed bounded Queue/Schedule/Appointments/Veterinarian/Quality summaries; no identifiers | read-only action routes only when section capability is allowed | loading/empty/error/stale/forbidden/partial-degraded specified, not implemented | D/T/M and accessibility behavior specified, not visually accepted | future default-off `CLINIC_V50_WORKSPACE_HOME`, dependent on `PORTAL_V50_SHELL` | 05B PostgreSQL authority/privacy/performance/OpenAPI matrix specified; runtime tests ABSTAIN in 05A | CONTRACT_READY | CONTRACT_READY / not IMPLEMENTED, TESTED or VISUALLY_VERIFIED |
