...
| CLN-004 | `prototype-v50/manifest.json#sha256` | Clinic / Appointments | `#clinic-appointments` → scoped `/appointments`; future nested `/:appointmentId` | default-off Portal registry plus backend cursor/index path implemented; administrative detail backend implemented, Portal detail absent; Queue unchanged | admin/reception `appointment.registry.read`; active exact clinic/location authority; detail reuses same capability/data category | list: scoped Portal BFF → registry GET; detail: exact-scope backend GET by appointment ID | list/detail read-only; detail actions empty; Queue and clinical commands remain separate | list states implemented; detail backend denial/nullability/technical-failure states tested; Portal L/E/C/T missing | list D/T/M evidenced; detail Portal D/T/M/accessibility contracted | existing registry flag reused; backend detail default off; future detail page+BFF absent | list evidence PASS; detail backend HTTP 13/13, registry 27/27, OpenAPI/migrations PASS | MODIFY | LIST_END_TO_END_IMPLEMENTED / DETAIL_BACKEND_IMPLEMENTED / PORTAL_MISSING |
