...
| CLN-005 | `prototype-v50/manifest.json#sha256` | Clinic / Patients | `#clinic-patients` → future `/clinics/:clinicId/locations/:locationId/patients` | association/consent/receipt/revision schema migrated but inactive; producer/API/UI absent | future `patient.admin.read`; exact clinic/location, active membership and purpose-specific consent; clinical access separate | versioned exact-location appointment-provenance association; temporal registry revisions; no owner contacts/medical data | read-only; detail/create/update/import/manual absent | L/Ø/E/C/degraded/no-results/access-denied contracted | D/T/M/a11y contracted, not implemented | future separate default-off patients flag; missing visibility/search policy fails closed | schema/concurrency/rollback integration suite; lifecycle writer still required | MODIFY | SCHEMA_IMPLEMENTED / WRITE_PATH_REQUIRED |
