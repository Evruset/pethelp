...
| CLN-004 | `prototype-v50/manifest.json#sha256` | Clinic / Appointments | `#clinic-appointments` → scoped `/appointments` | backend cursor registry and production-scale ordered query path implemented; Portal registry/detail absent; Queue unchanged | admin/reception `appointment.registry.read`; active exact clinic/location authority | `GET /v1/clinic/:clinicId/locations/:locationId/appointments`; signed snapshot/keyset cursor | registry read-only; existing Queue commands remain separate | L/Ø/E/C; upcoming/history, empty/error and privacy backend states; prototype check-in/done future | D/T/M Portal future | Portal rollout missing; existing queue flags unchanged | registry HTTP/PostgreSQL 27/27; query-index migration/plan 6/6; capability 26/26; build/OpenAPI/migration verify PASS | MODIFY | BACKEND_IMPLEMENTED / QUERY_INDEXED / PORTAL_MISSING |
