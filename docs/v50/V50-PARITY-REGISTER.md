...
| CLN-005 | `prototype-v50/manifest.json#sha256` | Clinic / Patients | `#clinic-patients` → future `/clinics/:clinicId/locations/:locationId/patients` | registry contract and hybrid association/consent decision complete; schema/API/UI absent | future `patient.admin.read`; exact clinic/location, active membership and purpose-specific consent; clinical access separate | appointment-provenance association, deduplicated active-pet allowlist; no owner contacts/medical data | read-only; detail/create/update/import/manual absent | L/Ø/E/C/degraded/no-results/access-denied contracted | D/T/M/a11y contracted, not implemented | future separate default-off patients flag; missing visibility/search policy fails closed | lifecycle/privacy/search/cursor/performance/rollback matrices defined | MODIFY | CONTRACT_COMPLETE / ASSOCIATION_SCHEMA_CONTRACT_REQUIRED |
