...
| CLN-005 | `prototype-v50/manifest.json#sha256` | Clinic / Patients | `#clinic-patients` → `/clinics/:clinicId/locations/:locationId/patients` | registry/detail implemented; exact-scope alias-only backend mutation implemented; Portal mutation UI absent | read uses `patient.admin.read`; local alias write uses `patient.admin.local-profile.update`; owner corrections and clinical access separate | signed read snapshot plus current privacy override; versioned clinic-local alias projection; owner master unchanged | read/navigation implemented; alias set/replace/clear backend implemented; create/import/lifecycle absent | read states plus mutation no-leak/version/idempotency/error matrix PASS | responsive read evidence PASS; alias mutation UI pending | existing Patients read flag plus independent default-off mutation flag | alias 12/12 covering E-01..E-28 plus concurrency/rollback/uniqueness; capability 35/35; Detail 8/8; Registry 8/8; build/OpenAPI/migration PASS | MODIFY | LOCAL_ALIAS_BACKEND_IMPLEMENTED / PORTAL_MISSING / OTHER_MUTATIONS_EXCLUDED |
| CLN-006 | `prototype-v50/manifest.json#sha256` | Clinic/Vet / Patient detail | `#clinic-patient` → scoped `/clinics/:clinicId/locations/:locationId/patients/:patientId` | administrative backend GET, scoped Portal page/BFF, strict parser and Registry navigation implemented; always-present local alias read projection is Portal-workflow ready; clinical record separate | admin read uses `patient.admin.read`; local alias write remains separately guarded; exact current association required | safe pet/relationship/bounded appointment projection plus exact-scope alias/version/timestamp; owner contacts and all clinical/document/financial fields excluded | read detail and alias backend implemented; Portal alias controls pending; clinical draft absent | current-state no-leak, technical, malformed, revoke-refresh, absent/cleared alias and degraded states pass | existing responsive detail evidence PASS; no alias UI yet | Patients read flag; alias read independent of mutation flag | backend detail 11/11 + Registry 8/8; Portal parser E1-P01..P11 3/3; backend build/OpenAPI and Node 22 Portal typecheck PASS | MODIFY | ADMIN_DETAIL_AND_ALIAS_READ_READY / PORTAL_ALIAS_WORKFLOW_PENDING / CLINICAL_POLICY_SEPARATE |
