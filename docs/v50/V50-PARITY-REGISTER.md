...
| OWN-002 | `prototype-v50/manifest.json#sha256` | Owner / Catalog | `#catalog` → `/owner/catalog` | flagged `OwnerCatalogV50Page`; legacy fallback retained | public/OWNER; optional JWT; foreign pet hint normalized | clinics, locations, services, freshness, fit reasons | typed booking intent only; no hold | loading/ready/map/filter/empty/error/stale/location fallback | 375/412/768/1440 captured | IMPLEMENTED / TESTED; VISUAL FAIL | backend 20/20 focused; Flutter 238/238 full; 12 runtime captures | REUSE | material prototype composition/hierarchy veto; default-off rollback |
...
| OWN-004 | `prototype-v50/manifest.json#sha256` | Owner / Clinic details | `#clinic` → `/owner/clinics/:clinicId` | flagged deep-linkable `OwnerCatalogV50Page` detail | public/OWNER; active public clinic/location/service only | clinic, location, pricing, freshness and confirmation | typed location/service handoff only | loading/ready/not-found/stale/no-services/error | 375/412/768/1440 captured | IMPLEMENTED / TESTED; VISUAL FAIL | focused Flutter/backend plus 12 runtime captures | MODIFY | prototype pricing-detail composition and mobile top band veto |
...
When any prototype file changes, regenerate the manifest, rerun inventory verification, diff screen/route/state identities, and update affected evidence before implementation continues. `V50-OWNER-02` repair package `v50-owner-02-c27e21f` passed the final independent gate; `OWN-009..OWN-011` are `VISUALLY_VERIFIED` and the counter is `3/30`. V50-OWNER-03 runtime and contracts are implemented/tested, but independent validation returned FAIL with one visual veto, so none of `OWN-002`, `OWN-004`, `OWN-018`, `OWN-019` is elevated to `VISUALLY_VERIFIED`.
