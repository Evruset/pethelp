...
| CLN-004 | `prototype-v50/manifest.json#sha256` | Clinic / Appointments | `#clinic-appointments` → scoped `/appointments`; future nested `/:appointmentId` | default-off Portal registry plus backend cursor/index path implemented; administrative detail contract complete but endpoint/UI absent; Queue unchanged | admin/reception `appointment.registry.read`; active exact clinic/location authority; detail reuses same capability/data category | list: scoped Portal BFF → registry GET; detail: future exact-scope GET by appointment ID | list/detail read-only; detail actions empty; Queue and clinical commands remain separate | list states implemented; detail L/E/C/T contract includes forbidden, stale-refreshed, nullable and unknown status | list D/T/M evidenced; detail D/T/M/accessibility contracted | existing registry flag reused; future detail page+BFF default off | list evidence PASS; detail authority/privacy/DTO/action/history matrix documented | MODIFY | LIST_END_TO_END_IMPLEMENTED / DETAIL_CONTRACT_COMPLETE / DETAIL_IMPLEMENTATION_MISSING |
