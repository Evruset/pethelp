...
## 2026-09-09 — OWNER-R2G Truthful Home and authoritative booking status

- Baseline: `e724fd7c87503270af132befef8b059617d19cde` (`origin/agent/owner-v50-r2f-real-journey`). Home now uses the neutral `Здравствуйте!` greeting and a new-booking action surface; it no longer claims that the Owner has no upcoming bookings. Catalog priority controls were removed and replaced with factual guidance about catalog contact/address, post-service informational price and post-service exact availability.
- Owner App completes the existing `nextAction=READ_STATUS` contract through `GET /v1/booking-holds/:holdId`. The read DTO is parsed fail-closed with exact top-level and nested keys, canonical public statuses only (`PENDING_CONFIRMATION`, `CONFIRMED`, `REJECTED`, `CANCELLED`, `EXPIRED`), and validated public summary/time/version fields. The UI reads the returned `holdId`, renders server-authored `statusTitle` and `safeDescription`, refreshes only that hold, preserves the last authoritative result on read failure and never turns status refresh into another POST. Unsupported `VIEW_APPOINTMENT` navigation is not exposed. Existing create idempotency-key reuse is preserved.
- Six actually-used V50 reference images are bundled in `apps/owner-app/assets/v50-reference/`; `cmp` and the focused byte-equality matrix PASS `6/6`. Runtime asset mapping contains no HTTP or GitHub URL, and imagery remains labelled as non-factual visual reference context.
- Node `v22.22.2`: clean `npm ci` PASS (`1054` packages installed); `npx expo install --check` PASS; typecheck PASS; lint PASS; full Jest PASS `30 suites / 180 tests` with the pre-existing `act()` diagnostics/open-handle debt recorded and `--forceExit` used after the verdict. Expo Doctor is `20/21`: the sole failure is absent local CocoaPods / recommended `1.15.2`, accepted only as `EXPO_DOCTOR=PASS_WITH_LOCAL_COCOAPODS_EXCEPTION`. Static Expo Web, iOS and Android exports PASS to `/tmp/pethelp-r2g-{web,ios,android}`. `git diff --check` PASS before documentation append.
- `HOME_UPCOMING_BOOKINGS_READ_MODEL=ABSENT`. R2G reads a known authoritative `holdId`; it does not add, infer or cache an Owner upcoming-bookings collection. Visual Product Owner review remains required; no real-device, signed-build, UAT or pilot evidence is claimed.

## 2026-09-09 — OWNER-R2H V50 experience restoration

- Parent is OWNER-R2G `d39cb3b4882bba698132bcceedc36b260a4d7b00`; the current canonical `prototype-v50/index.html` SHA-256 is `81d5effec9805021f9ad5c74d93519903ec6624895bacfe9282ad185a3c270c9`.
- Owner IA now preserves the same responsive `Главная / Клиники / Питомцы` navigation across Home, global Pets and Catalog. Global Pets is distinct from the booking/time pet selector, uses authoritative pet identity with initials when media is absent, and enters the published-result Diary. The absent bookings collection is not synthesized: `HOME_UPCOMING_BOOKINGS_READ_MODEL=ABSENT`.
- Home presents one dominant new-booking CTA with clinics, time and Diary as secondary tasks. Catalog and clinic/service surfaces retain V50 marketplace/card language while showing only current API name, address, contact, service, informational price and published availability. Decorative priority/sort controls, ratings, distance, scarcity and recommendations remain absent.
- The booking journey visibly progresses through `Питомец → Клиника и услуга → Время → Проверка`, retaining available authoritative context. Existing Back, stale-selection, conflict and retry behavior remains intact. Status still reads only the created hold, uses server-authored status text, keeps the last authoritative snapshot on refresh failure, and cannot recreate a booking; refresh is visually secondary.
- Fresh fixture-backed Playwright evidence under `/tmp/pethelp-r2h-visual-evidence/` covers Home/Pets/Catalog at `390×844` and Home/Catalog at `1440×900`, the complete mobile booking journey through Status, and overflow/target checks at 320 and 768 px. The independent recheck confirms stable selected navigation on Pets/Catalog, zero horizontal overflow, zero sub-44px measured targets, no failed requests and no application console errors. The only console warning is the pre-existing React Native Web `shadow*` deprecation.
- Node `v22.22.2`: clean `npm ci` PASS (`1054` packages), Expo dependency check PASS, typecheck PASS, lint PASS, focused Owner journey tests PASS `46/46`, and full Jest verdict PASS `31 suites / 181 tests`. Ordinary Jest retains the confirmed parent open-handle exit debt after the passing verdict. Expo Doctor is `20/21`; the only failure is local CocoaPods missing/below recommended `1.15.2`, recorded as `EXPO_DOCTOR=PASS_WITH_LOCAL_COCOAPODS_EXCEPTION`.
- Static Web, iOS and Android exports PASS to `/tmp/pethelp-r2h-{web,ios,android}-final`. The six bundled V50 assets remain byte-identical to their canonical source (`6/6`), runtime sources contain no remote asset URL, package/lockfiles have no drift, and no backend contract was changed. No real-device, signed-build, UAT or Product Owner visual sign-off is claimed.

## 2026-09-09 — OWNER-R2H-R1 experience repair

- Review → Availability restores the previously selected slot only when the current availability response contains the exact same slot ID and version. Availability → Service restores the service, and Service → Catalog restores the clinic. Restored display state never bypasses the existing service refresh or slot revalidation; choosing a different clinic clears retained service/time and choosing a different service clears retained time.
- User-facing and accessibility copy no longer exposes `V50 reference`, `backend`, `authoritative`, `API`, raw timezone identifiers or raw `amount RUB` formatting. Informational service price is formatted as a localised currency value while its wire contract is unchanged.
- Truthful global navigation follows `Главная / Питомцы / Клиники` without adding a bookings destination. Catalog, global Pets and Diary use stronger V50 marketplace, pet-card and health-history grouping while remaining limited to existing clinic, pet and published-result fields.
- Owner typecheck and lint PASS. Focused Home/Pets/Diary/Catalog/Service/Availability/Review tests PASS `9 suites / 58 tests`; final navigation subset PASS `2 suites / 11 tests`. Source scan finds no forbidden implementation vocabulary in user-facing TSX strings. Existing test-harness `act()` warnings remain.
- Browser revalidation initially stopped at an unavailable authenticated fixture session on port 8081; this is evidence-harness debt rather than an application verdict. No backend, schema, booking authority, same-hold status, status semantics, dependency or package contract changed.
- The established strict fixture harness subsequently PASSed the bounded `390×844` recheck: all three Back transitions visibly retain their exact selection, restored service and slot still trigger a fresh authoritative GET before moving forward, protected rendered text contains none of the implementation vocabulary, and there are no overflow, console, HTTP or request failures. Evidence is in `/tmp/pethelp-r2h-r1-visual/`; the existing React Native Web `shadow*` warning remains.

## 2026-09-10 — W7-D-R5R V50 visual, accessibility and Product-UX closure

- Git baseline is reconciled on clean starting HEAD `9c5715c3f6a26588cd5547c0eca31033a71bc207`: `origin/agent/w7-visit-result-diary` at `6118c73582aaf99eec4a892f53c2263b2ecc4154` is an exact ancestor (`0 37`), with no divergence. The canonical Clinic and Owner V50 reference files were not modified.
- Pre-repair production-browser evidence demonstrated bounded Clinic defects: raw `CAT` / `CONFIRMED`, shortened clinic/location UUIDs and implementation-facing server/capability copy, two page-level H1s, and an artificial 896 px content cap within a 1176 px desktop workspace. Owner Result detail omitted the available Amendment publication dates; its affected coloured-canvas captions also produced serious axe contrast failures at ratios `4.03–4.42`.
- Clinic presentation now localises the bounded visit species/status values without changing the DTO, removes user-facing identifiers and implementation vocabulary from the shell, leaves one route-owned H1, and expands the veterinarian list/detail workspace to the existing `max-w-6xl` rhythm. The list gains a compact, non-colour-only status pill and a two-column desktop composition. Owner Diary now displays each Amendment publication date and uses existing darker Owner tokens on affected coloured backgrounds. Visit completion, Result publication/readback, immutable Amendment order, capabilities and authority contracts are unchanged.
- Post-repair Clinic evidence at `390×844`, `430×932`, `768×1024`, `1024×900` and `1440×900` covers 20 state/viewport records plus the publish dialog. Raw enum/UUID/implementation-copy matches, duplicate H1s, horizontal overflow, targets below 44 px, console errors/warnings, failed requests and serious/critical axe violations are all zero. Dialog naming, focus containment and trigger return pass. Evidence: `/tmp/pethelp-w7-clinic-audit.erfZ1Q/evidence/` (27 files, 23 screenshots).
- Post-repair Owner evidence at the same five widths covers Diary list and Result detail in 10 page scans. Serious/critical axe violations, horizontal overflow/clipping, targets below 44 px, UUID exposure, failed requests and console errors are zero. Original Result precedes ordered Amendments, both fixture publication dates remain visible, and copy distinguishes Amendments from separate visits. Evidence: `/tmp/pethelp-w7-owner-audit.llhN2s/evidence/` (22 files, 20 screenshots). Five warnings are the pre-existing React Native Web `shadow*` deprecation only; all 20 controlled visual-fixture requests succeeded.
- Clinic TypeScript and production build PASS. Focused veterinarian Chromium gates PASS `20/20`; the affected shared-shell retry check PASS `1/1`. Owner TypeScript and targeted lint PASS; focused Diary Jest PASS `1 suite / 8 tests`; final Expo Web export PASS to `/tmp/pethelp-w7-r5r-owner-web-final`. `git diff --check` passes. Sticky backend/business and real-PostgreSQL evidence was not rerun because no authority, API, schema or migration file changed.
- Closure flags: `GIT_BASELINE_RECONCILED=PASS`, `V50_REFERENCE_PRESERVED=PASS`, `VISUAL_PARITY=PASS`, `RESPONSIVE=PASS`, `ACCESSIBILITY=PASS`, `PRODUCT_UX=PASS`, `FUNCTIONAL_REGRESSION=PASS`, `W7_D_R5R_COMPLETE=YES`.

## 2026-09-10 — OWNER-V50-R2I authoritative Home next action and active care

- Starting point was clean branch `agent/owner-v50-r2h-v50-experience-restoration` at `5ec2125895d4c162237617d87913d728cefcbc34`. W7-D-R5R remained sticky and was not reaudited. The existing backend `GET /v1/owner/home` contract was reused without changes to Backend, Booking Core, ranking, appointment contracts, OpenAPI, schema or migrations.
- Owner App now has a fail-closed Home adapter for exact `schemaVersion`, `serverNow`, `pets`, `selectedPet`, `selectionSource`, `nextAction` and `activeCare` shapes and the current finite action/status vocabularies. It rejects unknown keys/values, malformed selected-pet IDs and inconsistent selected-pet/active-care ownership before presentation. Native uses the established API client; Web uses the existing `/api/owner/*` bridge with a newly bounded `GET v1/owner/home` allowlist, including the optional `selectedPetId` query.
- Home query identity includes session `cacheScope` and the explicit current PetJourney selection. Backend selection remains authoritative: Home renders returned `selectedPet` name/species/breed/photo, retains the existing safe avatar when `photoUrl` is null, and does not independently rank or infer care. Session identity changes remount the authority-scoped Home and isolate/remove the prior Owner query family through the existing session cache boundary.
- The accepted V50 composition is unchanged. Only the Pet/next-action data surface is replaced: server-authored title/description/priority/deadline/action and valid active-care facts drive planned care, confirmed upcoming care and attention states. Priority is mapped to a localized badge and the authored deadline/start is formatted for display without exposing raw enum or ISO values. A refresh failure preserves React Query's last successful snapshot and adds a bounded retry notice; an initial failure renders a hard error. Client time is used only to format an already-authored timestamp, never to rank or transition state.
- `OPEN_CATALOG` enters the existing Catalog and `ADD_PET` enters the existing Pet journey. Since the current React Native app has no independent Home-addressable appointment, alternative-slot, telemed or emergency screen, `OPEN_APPOINTMENT`, `OPEN_ALTERNATIVE_SLOT`, `OPEN_TELEMED` and `OPEN_EMERGENCY` remain truthful informational states with no fabricated CTA or substitute navigation. Backend ranking was not altered.
- Focused parser/Home/navigation/Web bridge gates PASS `7 suites / 22 tests`; affected auth, PetJourney and root-layout compatibility gates PASS `3 suites / 19 tests`. Owner typecheck and targeted lint PASS. The existing React 19/RNTL overlapping-`act()` diagnostics and ordinary-Jest open-handle exit debt remain pre-existing harness debt; test verdicts pass. Final Expo Web export PASS at `/tmp/pethelp-owner-r2i-web-final`.
- Final controlled browser evidence uses verified DOM and PNG dimensions at `390×844` and `1440×900`: `14/14` captures cover planned/no-active-care, confirmed active care, booking attention, retained-snapshot failure, and four unsupported Pilot action codes. All `14/14` localized priority/deadline checks pass. Horizontal overflow, visible targets below 44 px, axe serious/critical violations, raw UUID/enums/ISO/implementation vocabulary, page errors, failed requests and unexpected application/console errors are all zero. The eight controlled HTTP 503 responses are the expected stale-refresh fixture. Latest-only evidence ZIP: `/tmp/pethelp-r2i-browser-latest.h237Tu.zip`, SHA-256 `247fd914cba35cb6aef58ce157a722b3bfb56c079567fcad7626bd770b915fb3` (17 files, 14 screenshots). Superseded wrong-viewport, pre-contrast and earlier pre-priority runs are excluded.
- Exit flags: `OWNER_HOME_API_INTEGRATION=PASS`, `OWNER_HOME_SELECTED_PET_AUTHORITY=PASS`, `OWNER_HOME_NEXT_ACTION=PASS`, `OWNER_HOME_ACTIVE_CARE=PASS`, `OWNER_HOME_PILOT_SCOPE_FENCING=PASS`, `OWNER_HOME_NAVIGATION=PASS`, `RESPONSIVE=PASS`, `ACCESSIBILITY=PASS`, `FUNCTIONAL_REGRESSION=PASS`, `OWNER_V50_R2I_COMPLETE=YES`.

## 2026-09-10 — OWNER-V50-R2I-R1 live prototype parity repair

- Source truth reconciled at expected HEAD `9a5fb32a517c81092648aac901db64846c27eaa1` with zero branch divergence. The pre-existing `apps/clinic-portal/next-env.d.ts` modification was preserved and is unrelated. R2I and W7 sticky contracts were not reopened; no schema, migration, availability, slot, hold, status-authority or clinical source changed.
- Pet selection now has Owner-session scope and survives clearing transient booking journey state. Cancel/finish clears continuation and journey state without clearing the current Owner pet; authoritative pet refresh removes a stale/deleted selection, and cache-scope changes prevent cross-Owner reuse. Home continues to send the scoped selected-pet ID.
- Catalog adds bounded client-side clinic/address search over the returned Owner catalog and explicitly labels each location as a branch, distinguishing same-name locations without inventing ratings, distance, availability or price facts absent from the location-list projection. The clinic decision page retains authoritative contact/services/prices and integrates the existing public Owner-compatible veterinarian projection. Doctor fields are fail-closed and limited to name, server-provided title and a freshness-safe indication that current published time exists; no timezone-free timestamp or doctor selection is fabricated.
- Post-submit status preserves cached booking context (pet, clinic/branch, service, local slot and displayed informational price), removes the raw status enum, explicitly tells a pending Owner that the clinic has not confirmed and they should not travel yet, and offers a bounded return to Home. Refresh failure retains the last authoritative snapshot and context.
- Owner typecheck and targeted lint PASS. Final focused Owner Jest PASS `7 suites / 57 tests`; existing React 19/RNTL `act()` diagnostics and ordinary Jest open-handle exit debt remain. Final Expo Web export PASS to `/tmp/pethelp-owner-r2i-r1-web-final`. `git diff --check` PASS.
- The local launcher was not modified. Expo was started with its required BFF environment externally, and a real seeded journey completed: Home → Жучка → distinct Demo Center branch → service → live slot → Review → authoritative pending status → Home. The final Home remains scoped to Жучка. Seven current full-page captures at `390×844` and `1440×900` are in `/tmp/pethelp-owner-r2i-r1-live/`; its `manifest.json` records branch distinction, status context and pet continuity PASS, with horizontal overflow `0`, visible targets below 44 px `0`, raw implementation identifiers `0`, axe serious/critical `0`, console/page errors `0` and failed requests `0`. Only the pre-existing RN Web `shadow*` warning appeared in the Expo process.
- Former R2I-R1 fixture/contract gap: the safe public doctor read uses `clinic_staff.id` consistently with the existing booking-selection `slot.staff_id` contract, while the prior rich-demo state had `0` active veterinarian staff rows and eight separate `catalog_schema.doctors` rows linked only through `slot.doctor_id`. The bounded follow-up below repairs the local fixture rather than changing that production identity contract.
- The prior independent-review veto was limited to missing live, booking-compatible rich-demo veterinarian evidence and is resolved by the fixture and live proof below.
- `OWNER-V50-R2I-R1-FIXTURE` resolves that gap without changing the production doctor API or Booking Core. Each of the eight existing demo humans now reuses one deterministic `92500000-*` UUID across `identity_schema.users`, `catalog_schema.doctors`, `clinic_schema.clinic_staff`, its active location-bounded `CLINIC_VETERINARIAN` membership, and both slot doctor bindings. The staff projection uses the existing `VETERINARIAN` staff role expected by the public read. Main and branch retain their original four-doctor assignments.
- Rich-demo reset now fences namespace-rooted aggregate producers in the canonical hold-before-slot order, re-locks the hold set after slot fencing, then locks appointments and outbox rows before deleting notification projections. It removes email deliveries, notifications, outbox/hold dependents, payment/clinical child chains, association projections, events, appointments, holds and finally `LOCAL_RICH_DEMO_V1` slots in the existing transaction. Ownership includes deterministic fixture IDs and runtime-created holds/appointments rooted in rich-demo slots; shared tables are not truncated and non-demo slots/holds remain outside the predicates. A pre-existing `925*` identity user is accepted only when its exact active location-bounded veterinarian membership already proves fixture ownership; otherwise seeding fails closed before granting authority.
- Two consecutive `seed rich-demo` → bounded `verify rich-demo` cycles PASS with workers enabled. Each verification reports memberships `expected=12 actual=12 missing=0 unexpected=0 duplicates=0` and doctors `mapped=8 futureDoctorSlots=104 futureStaffSlots=104 incompatibleSlots=0 missingCatalogDoctors=0`; a separate `DEMO_RESET=0` seed plus verification also PASS with the same counts. Namespace tests PASS `5/5`.
- Database acceptance: active rich-demo veterinarian staff `8`; main `4`; branch `4`; future OPEN doctor-bound slots `104`; future OPEN staff-bound slots `104`; doctor-bound slots missing compatible staff `0`; staff/location mismatch `0`; duplicate deterministic veterinarian identity `0`. Both live Owner doctor HTTP reads return `200`, the correct four location-specific names, no cross-location leakage, and `CURRENT` availability backed by future OPEN slots.
- Focused regressions PASS: backend public doctor/catalog read `4 suites / 16 tests`; Owner doctor adapter `1 suite / 2 tests`. Final blocked-slice Owner evidence is `/tmp/pethelp-owner-r2i-r1-fixture-live/`: four captures and `manifest.json` cover both locations at `390×844` and `1440×900`, with overflow `0`, visible targets below 44 px `0`, raw identifiers/roles/implementation vocabulary `0`, axe serious/critical `0`, console/page errors `0`, and failed requests `0`. The allowed pre-existing RN Web `shadow*` warning remains.
- Current flags: `OWNER_PET_CONTINUITY=PASS`, `OWNER_CATALOG_V50_PARITY=PASS`, `OWNER_CLINIC_DETAIL=PASS`, `OWNER_DOCTOR_DISCOVERY=PASS`, `OWNER_BOOKING_STATUS_CONTEXT=PASS`, `RESPONSIVE=PASS`, `ACCESSIBILITY=PASS`, `FUNCTIONAL_REGRESSION=PASS`, `OWNER_V50_R2I_R1_COMPLETE=YES`.

## 2026-09-13 — OWNER-R2J CI closure

- R2J product/runtime closure remains sticky: the authoritative My Bookings read model, server grouping, pagination, navigation, empty/error states, selected-pet continuity, responsive/accessibility behavior and functional regression evidence remain PASS. No production Owner runtime code, Booking Core, Backend B-01, Emergency Routing, Clinic Portal or database contract changed in this repair.
- PR #79 CI reproduced a single repository blocker under Node 22: Expo SDK 57 expected six newer compatible patch releases than the versions declared and locked (`expo` `57.0.22`, `expo-constants` `57.0.18`, `expo-linking` `57.0.10`, `expo-router` `57.0.21`, `expo-secure-store` `57.0.4`, and `expo-splash-screen` `57.0.9`). There is no `patch-package` setup or stale patch in Owner App; `npm ci` itself was already deterministic and successful.
- The six direct dependencies were aligned to Expo's reported SDK 57 compatibility versions and `apps/owner-app/package-lock.json` was regenerated by the standard Expo/npm installer. No warning suppression, exclusion, override, forced install or SDK upgrade was used.
- Local Node `v22.22.2` / npm `10.9.7`: clean `npm ci` PASS; Expo dependency validation PASS (`Dependencies are up to date`); full Jest verdict PASS `41 suites / 216 tests`; typecheck PASS; lint PASS; test/pilot public config PASS; native prebuild PASS; and `git diff --check` PASS. Local macOS Expo Doctor is `20/21` solely because local CocoaPods is unavailable/outdated; the Linux Owner CI native-tooling check passes and is the authoritative full Expo Doctor gate for PR #79.
- A bounded repair commit exists locally. The environment's VetHelp guard rejected the authorized conditional push, so PR #79 remains at its prior head until a human runs the exact push command or supplies unconditional push authorization; remote CI has therefore not yet validated the repair.
