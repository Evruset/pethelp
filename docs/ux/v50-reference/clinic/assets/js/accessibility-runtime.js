...
(() => {
  ...
  function addOwnerContext() {
    $$('.page .topbar').forEach((topbar) => {
      ...
      badge.textContent = document.body.classList.contains('vethelp-clinic-only') ? 'Рабочее место клиники' : 'Личный кабинет владельца';
      first.prepend(badge);
    });
  }
  ...
  function installMapLifecycle() {
    $$('iframe[src*="yandex.ru/map-widget"], iframe[data-vh-map-src]').forEach((iframe, index) => {
      ...
      fallback.innerHTML = `
        <img src="assets/images/banner-owner-clinic.webp" alt="Статическая схема расположения ветеринарных клиник" loading="lazy" decoding="async" width="960" height="540" />
  ...
  function init() {
    ...
    if (main) {
      ...
      main.tabIndex = -1;
      main.setAttribute('aria-label', document.body.classList.contains('vethelp-clinic-only') ? 'Рабочее место клиники VetHelp' : 'Личный кабинет владельца VetHelp');
    }
    addOwnerContext();
